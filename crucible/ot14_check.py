"""OT-14: feedback-free staleness on the F2 dial (qualified v3
configuration), constants per PREREG-OT14-APPENDIX.md. Refuses to run
until the appendix is SEALED.

    .venv/Scripts/python crucible/ot14_check.py
"""
...
N_RESAMPLE = 10
FLOOR_REDRAWS = 5
OUT = os.path.join(HERE, "..", "results", "OT14-staleness-dial.json")
APPENDIX = os.path.join(HERE, "PREREG-OT14-APPENDIX.md")


def require_seal():
    text = open(APPENDIX, encoding="utf-8").read()
    if "STATUS: DRAFT-UNSEALED" in text or "STATUS: SEALED" not in text:
        sys.exit("REFUSED: PREREG-OT14-APPENDIX.md is not SEALED. "
...
def main():
    ...
    def op(queries):
        # caveat (i): the dot consumer's operator is analytic
        return queries.T @ queries / len(queries)

    # dial -- caveat (ii): tau=0 reference is the full fixed cs pool
    ...
    # lever -- stale codec frozen at P(0); fresh refreshed per stratum
    stale = quantize_against(index, p0, BITS)
    strata = {}
    for t in nz:
        p_t = op(stratum(t, N_DIAL, SEED + int(t * 10000) + 1))
        fresh = quantize_against(index, p_t, BITS)
        ex10, s10, f10, ex1 = [], [], [], []
    ...
    json.dump({"claim": "OT-14", "seed": SEED, "floor": floor,
               ...
               "verdict": verdict},
              open(OUT, "w"), indent=1)
    print(f"\nOT-14: {verdict} -> {os.path.relpath(OUT, HERE)}")
