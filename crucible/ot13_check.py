"""OT-13: the two floor curves on the F1 family, constants per
PREREG-OT13-APPENDIX.md. Refuses to run until the appendix is SEALED.

    .venv/Scripts/python crucible/ot13_check.py
"""
...
SKETCH = 80
EPS = 1e-3
M_GRID = [3000, 1000, 562, 316, 178, 100, 56, 30, 10, 1]
BAND = [m for m in M_GRID if 30 <= m <= 1000]
HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "..", "results", "OT13-floor-curves.json")
APPENDIX = os.path.join(HERE, "PREREG-OT13-APPENDIX.md")


def require_seal():
    text = open(APPENDIX, encoding="utf-8").read()
    if "STATUS: DRAFT-UNSEALED" in text or "STATUS: SEALED" not in text:
        sys.exit("REFUSED: PREREG-OT13-APPENDIX.md is not SEALED. "
...
def main():
    ...
    for m_mult in M_GRID:
        ...
        def consumer(x, step=step):
            s = w @ np.asarray(x, float) / TEMP
            s = s - s.max()
            e = np.exp(s)
            return float(np.round((e[0] / e.sum()) / step) * step)

        p_hat = blind_probe(consumer, pts, mode="lstsq",
                            sketch_dim=SKETCH, eps=EPS,
                            rng=np.random.default_rng(SEED),
        ...
        for s_pair, v_pair in zip(sigmas, cache):
            dd, per_draw = [], []
            for vp in v_pair:
                sq = (q(vp) - q0[:, None]) ** 2
            ...
            pred = np.sign(np.trace(p_hat @ (s_pair[0] - s_pair[1])))
    ...
    mc2 = all(c["straddles"] for c in mc1)
    ...
    print(f"MC2 straddle at every interior step: "
          f"{'ok' if mc2 else 'VOID'}")
