"""OT-7: numerical verification of the transformation and invariance
taxonomy sealed in OT-CRUCIBLE.md, constants per PREREG-OT7-APPENDIX.md.

    .venv/Scripts/python crucible/ot7_check.py
"""
...
OUT = os.path.join(HERE, "..", "results", "OT7-invariance.json")
...
def run_cell(rng, d, r):
    ...
    # rank-1 has one angle/eigenvalue; energy_rank of rank-1 is degenerate
    # under GL (always 1) -- degenerate cells are skipped per the rule
    # "non-invariance demonstrated where the quantity can move".
    frag_keys = [k for k in fr
                 if not (r == 1 and k in ("energy_rank",))]
    ok_frag = all(out["frag_gl"][k] >= FRAG_SHARE for k in frag_keys)
    ok_o = all(v <= TOL_O for v in io.values())
