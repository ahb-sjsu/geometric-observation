"""OT-13 v2: the two floor curves on the F1 family, constants per
PREREG-OT13-APPENDIX-V2.md — smooth-consumer probe (OT-5 precedent),
family-validated grid. Refuses to run until the v2 appendix is SEALED.

    .venv/Scripts/python crucible/ot13_check_v2.py
"""
...
OUT = os.path.join(HERE, "..", "results", "OT13-floor-curves-v2.json")
APPENDIX = os.path.join(HERE, "PREREG-OT13-APPENDIX-V2.md")


def require_seal():
    text = open(APPENDIX, encoding="utf-8").read()
    if "STATUS: DRAFT-UNSEALED" in text or "STATUS: SEALED" not in text:
        sys.exit("REFUSED: PREREG-OT13-APPENDIX-V2.md is not SEALED.")
...
def main():
    ...
    json.dump({"claim": "OT-13-v2", "seed": SEED, "rms": rms,
