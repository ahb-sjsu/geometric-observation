"""OT-7 v2: constants per PREREG-OT7-APPENDIX-V2.md (degenerate-cell
annotation, integer-quantity bar 0.50, GL cond cap 1e2). Diagnosis of
the as-executed v1 FAIL: OT7-NOTES.md.

    .venv/Scripts/python crucible/ot7_check_v2.py
"""
...
OUT = os.path.join(HERE, "..", "results", "OT7-invariance-v2.json")
...
def run_cell(rng, d, r):
    ...
    def bar(k):
        return 0.50 if k == "energy_rank" else FRAG_SHARE
    ok_frag = all(out["frag_gl"][k] >= bar(k)
                  for k in fr if k not in degenerate)
    ok_o = all(v <= TOL_O for k, v in io.items() if k not in degenerate)
