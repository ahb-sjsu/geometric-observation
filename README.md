...
| **GO-11** the exact region behind the tax ([manuscript](paper/go11-conditional-region.tex)) | `[demonstrated]` ✅ | the conditional rate–work region solved where the tools reach: the **first vector-Gaussian common-reconstruction-type function** (closed form ½log₂ g\*, with g\* the larger root of Dsg² − (D+s−ρ²)g + (1−ρ²), s = 1+τ²), the m=1 frontier as a two-water-level system with the **misalignment theorem** — a strict rate–work tradeoff opens whenever the reset context is misaligned with the read (ρ² strictly between 0 and 1); Paper V's single-corner collapse is exactly the alignment degeneracy — and the m=2 region as a nine-parameter matrix program from which GO-10's tax-gap formula is *derived*. Six results (incl. Theorem 6: the work floor is attained iff the context is X-measurable-and-SDC or plane-blind — correcting the conjectured Σ_{Y\|S} to the encoder-accessible Σ_{Y\|V}), each R-IND-5-verified **before** assertion; C3 harness **ALL PASS 6/6** (060); and the **encoder-side tilt measured** (061, PASS 6/6): a record tilted into the context's X-measurable direction carries a 0.595 b/sym larger context-decode discount at matched consumer distortion, its content at the Gray floor, while the analytically context-blind flipped record shows S-dependence 0.000 exactly — records optimally *written in ink the eraser can read*, the encoder-side complement of 058/059's eraser-side tilt |
