...
| **GO-11** the exact region behind the tax ([manuscript](paper/go11-conditional-region.tex)) | `[predicted]` ✅ | the conditional rate–work region solved where the tools reach: the **first vector-Gaussian common-reconstruction-type function** (closed form ½log₂ g\*, with g\* the larger root of Dsg² − (D+s−ρ²)g + (1−ρ²), s = 1+τ²), the m=1 frontier as a two-water-level system with the **misalignment theorem** — a strict rate–work tradeoff opens whenever the reset context is misaligned with the read (ρ² strictly between 0 and 1); Paper V's single-corner collapse is exactly the alignment degeneracy — and the m=2 region as a nine-parameter matrix program from which GO-10's tax-gap formula is *derived*. Five results, each R-IND-5-verified **before** assertion; C3 harness **ALL PASS 6/6** (060, governed); first seal under the full §5.1 power-first regime (pilot forced zero bar changes). Named future faces: the encoder-side tilt prediction (operational); Conjecture 3's iff |
