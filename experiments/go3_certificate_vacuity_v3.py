...
def main():
    ...
    # transition-width-honest bands (threshold +/- 0.25, ~sigmoid width), not a step
    hi_ok = all(c["recall"] >= 0.75 for c in corpora if c["rho"] >= 1.25)
    lo_ok = all(c["recall"] <= 0.25 for c in corpora if c["rho"] <= 0.75)
    ...
    out = {
        "claim": "GO-3 certificate vacuity v3", "prereg": "GO-P-2026-014",
