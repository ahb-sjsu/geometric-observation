# Geometric Observation — GO-5 v2: the alpha=1 density/hubness quotient restores
# invariant fidelity in a NON-SPECTRAL domain (locally-scaled NN retrieval).
# Governed by prereg/GO-P-2026-017 (sealed a1c9faa9; registered commit 44bef8a).
# Corrects v1: GLOBAL-scale kernel (density kept in the affinity so raw retrieval
# is hubbed) + a baseline-hubness regime gate. MIT License.
...
def _run(seed, uniform=False):
    ...
    eps = float(np.median(np.sort(Dx, axis=1)[:, 6]))             # GLOBAL bandwidth = median 7th-NN sqdist
    W = np.exp(-Dx / (eps + 1e-12)).astype(np.float32)           # density stays in the affinity
    np.fill_diagonal(W, 0.0)
    q = W.sum(1) + 1e-12                                          # density (varies with local density)
...
def main():
    ...
    baseline_hub = [res[s][0.0]["hub_skew"] >= 0.5 for s in seeds]

    verdict = {
        "baseline_hubness": bool(sum(baseline_hub) >= 2),
    ...
    out = {
        "claim": "GO-5 alpha=1 density quotient (non-spectral, global scale)", "prereg": "GO-P-2026-017",
