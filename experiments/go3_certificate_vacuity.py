# Geometric Observation — GO-3: certificate vacuity predicts single-stage retrieval death.
# Governed by prereg/GO-P-2026-012 (sealed 34f964fa; registered commit pending).
# PROSPECTIVE. A margin certificate mu_hat with a DERIVED vacuity threshold
# mu_crit = E[max of (N-1) standard normals] predicts where recall@1 collapses
# (rho = mu_hat/mu_crit ~ 1), across >=5 structural corpus families. MIT License.
"""GO-3: does the certificate's vacuity threshold locate where retrieval dies?"""
...
def _corpus(family, N, D, sigma, seed):
    ...
    t = r.integers(0, N, QN)
    z = r.standard_normal((QN, D)).astype(np.float32)
    q = _unit(e[t] + sigma * z)
    S = q @ e.T                                  # (QN, N)
...
def main():
    ...
    out = {
        "claim": "GO-3 certificate vacuity", "prereg": "GO-P-2026-012",
