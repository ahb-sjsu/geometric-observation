# Geometric Observation — GO-2 positive half on attention keys, v5 (closer).
# Governed by prereg/GO-P-2026-005 (sealed bb6ad838; registered commit 96186b3).
# Gates on the DIRECT projected error covariance proj_var = tr(P_C Sigma_delta)/
# tr(P_C Sigma_0) (no query-sampling noise), with a concentrated r=4 read subspace
# so the a-vs-b flip is decisive. MIT License.
"""GO-2 v5: proj_var gated directly; the consumer-projected-covariance mechanism."""
...
def main():
    ...
    out = {"claim": "GO-2 kv-keys v5", "prereg": "GO-P-2026-005", "n_seeds": SEEDS, "arms": ARMS,
    ...
    verdict = {
        ...
        "proj_ranks_both": bool(iso["median_spearman_projvar_kl"] >= 0.9 and sub["median_spearman_projvar_kl"] >= 0.9),
        "flip_decisive": bool(
            iso["median_adv_kl_logbovera"] >= np.log(1.3) and sub["median_adv_kl_logbovera"] <= -np.log(1.15)
            and iso["kl_a_lt_b_seeds"] >= 11 and (SEEDS - sub["kl_a_lt_b_seeds"]) >= 11),
