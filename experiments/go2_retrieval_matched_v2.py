# Geometric Observation — GO-2 instance 2' clean close: retrieval matched probes.
# Governed by prereg/GO-P-2026-009 (sealed 0c91db9e; registered commit 7629a05).
# PROSPECTIVE. Fixes GO-P-2026-008's mis-specified SNR gate: injects a
# 90%-subspace + 10%-complement error mix (no exact-zero arm) and gates the
# LOSER's distortion. MIT License.
"""GO-2 instance 2' clean: eta-mix matched-distortion probes on retrieval ranking."""
...
def _inject(E, eps, Vsub, seed_off, seed):
    """Add per-item error of norm eps_i: sqrt(1-ETA) in span(Vsub) + sqrt(ETA) in its
    orthogonal complement (so ||delta_i|| = eps_i exactly; 10% leaks isotropically)."""
    ...
    s = u @ Vsub                              # unit, in span(Vsub)
    g = rng.standard_normal((N, D)).astype(np.float32)
    g -= (g @ Vsub.T) @ Vsub                  # project onto complement of span(Vsub)
    g /= (np.linalg.norm(g, axis=1, keepdims=True) + 1e-8)   # unit, in complement
    delta = (np.sqrt(1 - ETA) * s + np.sqrt(ETA) * g) * eps[:, None]   # ||.|| = eps_i
...
def main():
    ...
    out = {"claim": "GO-2 retrieval matched-distortion clean (instance 2')", "prereg": "GO-P-2026-009",
           "n_seeds": SEEDS, "eta": ETA, "arms": ARMS, "median_recon": med(RC), "consumers": {}}
    for C in CONSUMERS:
    ...
    # loser = the distorted (worse) arm under each consumer; winner = the ~blind arm
    loser_dO = {"read_A": max(rA["median_dO"][a], rA["median_dO"][b]),
                "read_B": max(rB["median_dO"][a], rB["median_dO"][b])}
    winner_dO = {"read_A": min(rA["median_dO"][a], rA["median_dO"][b]),
                 "read_B": min(rB["median_dO"][a], rB["median_dO"][b])}
    min_loser_dO = min(loser_dO.values())
    ...
    verdict = {
        ...
        "snr_loser": bool(min_loser_dO >= 10 * max_floor),
        "flip_decisive": bool(
    ...
    out["diagnostics"] = {
        ...
        "loser_dO": loser_dO, "winner_dO_blind_arm": winner_dO,
        ...
        "snr_min_loser_dO": min_loser_dO, "snr_max_floor": max_floor,
    }
    out["verdict"] = verdict
