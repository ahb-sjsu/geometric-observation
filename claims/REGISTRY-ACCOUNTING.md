...
| 058 | GO-10 operational face: complementarity tax as decode thresholds on materialized codebook records (coordinated-split joint reset; Gaussian orthogonal instance; gap vs (1/2)log2(1/(s^2+(1-s^2)d^))) | cost | sealed 2026-08-03 (hash 69846466...); one pre-seal pilot logged in-prereg (seed 20260818: gaps 0.255/0.113 vs shuffled 0.005/0.009; W4 reshaped from absolute-tracking to the GO-9 fraction-window pre-seal, all margins >= 1.3x per the power-first rule); **governed run pending** (seed 20260820, T=400) | (pending) | GO10-operational-tax.json (pending) |
