...
| GO-2 (pos. half: consumer-projected covariance *controls*) | Downstream preservation is controlled by the error covariance projected on the consumer's read subspace, tr(P_C·Σ_δ). | `[demonstrated]` (instance 1) | 6–8 | [GO-P-2026-006](../prereg/GO-P-2026-006-kv-keys-flip-focused.md) | `experiments/go2_kv_keys_v6.py` | [GO2-kv-keys-v6.json](../results/GO2-kv-keys-v6.json) | B | ✅ **5/5 gated** | 1 / ≥2 | [v6 notes](../experiments/GO2-kv-keys-v6-NOTES.md): **PASS** — symmetric flip (iso a 2.5×, sub b 1.56×, **12/12 both**), `proj_var` tracks the flip sign **12/12 both**, recon can't (Spearman 0.40 vs 0.80). Diagnostics (ungated): anti-probe 21.4/6.34 (both ≥5), NEG-9 middle-pair persists (iso full-rank 0.80). **Instance 2 (retrieval, GO-P-2026-007, prospective) did NOT reproduce the discriminating flip** — arm b Pareto-dominated on recon, so no dissociation (NEG-10); precondition = recon-matched arms. **Instance 2′ (GO-P-2026-008, prospective, recon-matched probes): mechanism demonstrated in substance** — total flip at *identical* recon (0.0964), read_A→a distorted/b=0, read_B→reverse (12/12), proj_var tracks 12/12, recon blind (Spearman 0.40); but registered flag False on a **mis-specified SNR gate** (min-arm dO=0 by perfect orthogonality). `[replicated]` not yet claimed → GO-P-2026-009 (fix gate + non-degenerate injection). Priors: NEG-5…10 |
