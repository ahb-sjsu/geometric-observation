...
| GO-2 (pos. half: consumer-projected covariance *controls*) | Downstream preservation is controlled by the error covariance projected on the consumer's read subspace, tr(P_C·Σ_δ). | `[replicated]` (**open**, core mechanism decisive) | 6–8 | [GO-P-2026-005](../prereg/GO-P-2026-005-kv-keys-projected-covariance.md) | `experiments/go2_kv_keys_v5.py` | [GO2-kv-keys-v5.json](../results/GO2-kv-keys-v5.json) | B | **5/7 (secondary ❌)** | 0 / ≥2 | [v5 notes](../experiments/GO2-kv-keys-v5-NOTES.md): **core decisive** — symmetric flip (iso a 2.5× / sub b 2.7×, **12/12 both**), `proj_var` tracks the flip sign **12/12 both consumers**, recon can't (Spearman 0.40). Missed 2 **secondary** gates: anti-probe ≥5-both (4.0 under r=4) and full-4-arm Spearman ≥0.9 (0.80, a *middle* b-vs-d pair → NEG-9). Re-test → GO-P-2026-006 (gate the flip exactly; anti-probe/full-rank as diagnostics). NEG-8, NEG-6, NEG-5 |
