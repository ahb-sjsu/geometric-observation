...
map from a representation to a task output, together with a metric on that output. A
second-order expansion of the downstream distortion isolates a single leading-order
controlling quantity: the reconstruction error read through the consumer, namely the
trace of the error second moment weighted by a positive semidefinite read operator
that is the pullback of the output metric along the consumer. Ordinary reconstruction
fidelity is the isotropic corner in which this operator is the identity, where the
Gaussian mean squared error specialization of Shannon rate distortion and Gauss least
squares reside. We prove that the read operator is the pullback geometry of
distinguishability and induces a quotient of the representation by what the consumer
cannot separate; that it is recoverable from black box consumer queries under a
specified observation distribution; and that it defines a consumer relative rate
distortion function whose achievability and converse we establish for a general
source, together with a coding theorem for the true nonlinear consumer divergence that
reduces exactly to coding of the consumer output. We instantiate the read operator for
attention, gradient optimization, and retrieval, and position the framework as the
...
predeclared acceptance bar; four predictions are confirmed and one is refuted and
...
stays blind---the recurring signature of Observation Theory.}
\label{fig:flip}
\end{figure}
...
boundary. A prospective trained-model (Llama-3.2-3B) hardening of GO-2 missed its
sealed bar and is discussed in the text.}
