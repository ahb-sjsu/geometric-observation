...
a three-node ensemble under a sealed pre-registration, the \emph{false-clear
rate} of the local-read certificate. The result is a clean statement of
...
but ``the consumer's footprint is caught up.''
\end{abstract}
...
Table~\ref{tab:res} reports the sealed measurement. On the same delayed follower,
the hot-footprint local read is stale $\approx99\%$ of the time, while the cold
footprint reads $\approx1\%$ stale; \texttt{sync()} holds the false-clear at
zero; the mean leader--follower \texttt{zxid} gap is ${\sim}12$. The bars and all
manipulation checks pass on every seed.

\begin{table}[t]
\caption{Sealed XPROTO-ZK measurement (three graded seeds). ``naive-hot''/
...
substrate guards), the ensemble scripts, and the graded record are archived with
the program~\cite{bond_prereg}; the substrate is unmodified ZooKeeper~3.9.
