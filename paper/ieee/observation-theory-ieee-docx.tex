...
bottleneck. We further prove an exact two-observer coding theorem: a Gaussian source is
successively refinable across two read operators if and only if their optimal error
covariances nest, the excess rate given by a second max-det program. Every empirical
claim is sealed-registered against a preset bar; four of five predictions are
confirmed, one refuted, and a corrected trained-model test passed.
\end{abstract}

\medskip\noindent\textbf{Index Terms---}
Distinguishability, indirect source coding, information bottleneck, information
geometry, neural network compression, quantization, rate--distortion theory,
reproducibility, source coding, successive refinement.


\IEEEpeerreviewmaketitle

\noindent\emph{Department of Computer Engineering, San Jos\'e State University, San Jos\'e, CA 95192 USA. Email: andrew.bond@sjsu.edu. ORCID: 0009-0003-2599-6158.}


\section{Introduction}

Shannon fixed a distortion measure and asked for the fewest bits meeting it
[1], [2]; Gauss fixed the same measure and asked for the
least-squares estimate [3]. This paper reads their common assumption
...
[5], [6], [7]; its lossless, discrete form is
coding for computing and graph entropy [8], [9]; its modern
face is the rate--distortion--perception tradeoff [10] and task-oriented
/ semantic communication [11]; and its deployed form, for neural
networks, is loss-aware (Hessian-weighted) compression, from Optimal Brain
Damage/Surgeon [16], [17] to Hessian-aware quantization and GPTQ
[18], [19]. What is missing across these is a single,
...
bottleneck [12] (an arbitrary, non-geometric relevance variable), as
...
changes but the consumer; and (vi) an exact two-observer successive-refinement
theorem---a Gaussian source refines across two distinct read operators if and only if
their optimal error covariances nest, with a closed-form max-det rate loss when they do
not (\S\ref{sec:multiobs}).
The three companion works are Paper~I (the angular observer and the spectral
budget-inversion mechanism) [20], Paper~II (geometric key quantization and
the reconstruction-is-not-the-target negative) [21], and the claim ledger
[22]; this is their information-theoretic synthesis.
...
\centering
\footnotesize
\caption{TABLE I. Observation Theory as the identifiable geometric middle term between two
classical extremes. The read operator $\PC$ is the axis.}
\label{tab:positioning}
\setlength{\tabcolsep}{6pt}\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}p{3.0cm}p{2.1cm}p{6.2cm}@{}}
\toprule
...
operator will turn out to be the query second moment $\PC=\E[qq^\top]$
(\S\ref{sec:readop}), and our experiments make the consequences concrete: two $4$-bit
...
\centering
\footnotesize
\caption{TABLE II. The read operator $\PC$ instantiated for three consumers. Ordinary
reconstruction is the isotropic corner $\PC=I$.}
\label{tab:readops}
\setlength{\tabcolsep}{6pt}\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}p{2.0cm}p{2.3cm}p{4.3cm}p{4.1cm}@{}}
\toprule
...
collapse of Paper~II [21]: a truly common error component is free by shift
...
\caption{Fig. 1. The flip. Two lossy codes produce errors $\delta_A,\delta_B$ of \emph{equal}
...
\noindent\textbf{Dissociation.} Two 4-bit codes at an audited matched budget (spread
...
\noindent\textbf{Flip.} Holding reconstruction fixed and changing only the query bank, the
...
\noindent\textbf{Replication and out-of-sample.} The flip replicates on retrieval ranking
...
\caption{Fig. 2. Capacity (GO-3), from the committed result artifact. The derived margin
...
spectrally to the manifold operator [14], so the fixed low-$\lambda$
...
Paper~I [20]). Convergence does not force step-wise monotonicity in $N$; the
...
geodesic fidelity is nonincreasing---Paper~I's registered inversion [20]. We
...
Shannon's Gaussian $R(D)$ [2]; the estimation dual is Gauss--Markov
least squares [3].
\end{proposition}
\begin{proof}
...
[8], [9], here given a geometric read operator.
\end{remark}
...
the information bottleneck [12]---which optimizes a different object (a
...
with per-stage distortion measures is Rimoldi's [32]: $(R_1,R_2)$ achievable
...
[28], [33] under the reference letter $\hat x_0=0$ (which has
...
[32], [28], [33] makes the corner $(R_1(D_1),R_2(D_2))$ achievable.
\end{proof}
...
Equitz--Cover [31]; conflict requires genuinely distinct observers. \emph{(iii) Kernel overhead.} If the base layer codes a direction in
...
with the ledger [22]): the nesting characterization, the Equitz--Cover
...
\emph{Related work.} The successive-refinement line is Koshelev [27],
Equitz--Cover [31], Rimoldi [32] (per-stage measures may
differ), and, for abstract sources, Effros [28] and Kostina--Tuncel
[33], which supply the operational region invoked above. Two works are
closest, and neither subsumes ours. Nayak \emph{et al.}~[29] treat one
...
refinability can fail; Op~'t~Veld and Gastpar [30] refine a rank-deficient
...
density and recovers density-free geometry [13]: there the density mode
...
correction [15]). It does not, across four prospective tests
...
[21], the motivating negative). The flip is observable only for
...
\centering
\footnotesize
\caption{TABLE III. The falsifiable core. Each row is a sealed, before-the-fact registration with
...
\setlength{\tabcolsep}{6pt}\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}p{1.3cm}p{4.2cm}p{1.7cm}p{3.6cm}@{}}
\toprule
...
reference-letter moment [4]; we recall it in \emph{information-density}
...
$d_{\PC}$ [23]. Nothing about the nonlinearity of
...
[24]), $R_{\mathcal{C}}^{\mathrm{true}}(D)=R_U(D)$ for every $D\ge0$.
\end{theorem}
\begin{proof}
...
of Linder--Zamir [25]: $q_G(u,\hat u):=\tfrac12(\hat u-u)^\top
...
Linder--Zamir admissibility conditions [25] for $(U,D_Y)$ (labels follow [25], whose (c),(d) do not bind here): \emph{(a)}
...
theory of Kostina--Verd\'u [26] should port under their regularity
conditions.
\end{remark}
...
public companion repository [22]; each registry entry is committed
...
\item  C.~E. Shannon. Coding theorems for a discrete source with a
fidelity criterion. \emph{IRE Nat.\ Conv.\ Rec.}, 4:142--163, 1959.
\item  C.~F. Gauss. \emph{Theoria Motus Corporum Coelestium}, 1809.
\item  T.~Berger. \emph{Rate Distortion Theory}. Prentice-Hall, 1971.
\item  R.~L. Dobrushin and B.~S. Tsybakov. Information transmission
with additional noise. \emph{IRE Trans.\ Inf.\ Theory}, 8(5):293--304, 1962.
\item  J.~K. Wolf and J.~Ziv. Transmission of noisy information to a
noisy receiver with minimum distortion. \emph{IEEE Trans.\ Inf.\ Theory},
16(4):406--411, 1970.
\item  H.~S. Witsenhausen. Indirect rate distortion problems.
\emph{IEEE Trans.\ Inf.\ Theory}, 26(5):518--521, 1980.
\item  J.~K\"orner. Coding of an information source having ambiguous
alphabet and the entropy of graphs. \emph{Trans.\ 6th Prague Conf.}, 411--425, 1973.
\item  A.~Orlitsky and J.~R. Roche. Coding for computing.
\emph{IEEE Trans.\ Inf.\ Theory}, 47(3):903--917, 2001.
\item  Y.~Blau and T.~Michaeli. Rethinking lossy compression: The
rate--distortion--perception tradeoff. \emph{ICML}, 2019.
\item  D.~G\"und\"uz et al. Beyond transmitting bits: Context,
semantics, and task-oriented communications. \emph{IEEE J.\ Sel.\ Areas Commun.},
41(1):5--41, 2023.
\item  N.~Tishby, F.~C. Pereira, and W.~Bialek. The information
bottleneck method. \emph{Proc.\ 37th Allerton Conf.}, 368--377, 1999.
\item  R.~R. Coifman and S.~Lafon. Diffusion maps. \emph{Appl.\
Comput.\ Harmon.\ Anal.}, 21(1):5--30, 2006.
\item  J.~Calder and N.~Garc\'ia Trillos. Improved spectral
convergence rates for graph Laplacians on $\varepsilon$-graphs and $k$-NN graphs.
\emph{Appl.\ Comput.\ Harmon.\ Anal.}, 60:123--175, 2022.
\item  M.~Radovanovi\'c, A.~Nanopoulos, and M.~Ivanovi\'c. Hubs in
space. \emph{J.\ Mach.\ Learn.\ Res.}, 11:2487--2531, 2010.
\item  Y.~LeCun, J.~S. Denker, and S.~A. Solla. Optimal Brain Damage.
\emph{NeurIPS}, 1990.
\item  B.~Hassibi and D.~G. Stork. Second order derivatives for
network pruning: Optimal Brain Surgeon. \emph{NeurIPS}, 1993.
\item  Z.~Dong et al. HAWQ: Hessian aware quantization of neural
networks. \emph{ICCV}, 2019.
\item  E.~Frantar, S.~Ashkboos, T.~Hoefler, and D.~Alistarh.
GPTQ: Accurate post-training quantization for generative pre-trained transformers.
\emph{ICLR}, 2023.
\item  A.~H. Bond. The angular observer: geodesic preservation and the
mode-budget inversion. Companion Paper~I, 2026, doi:~10.5281/zenodo.21423308.
\item  A.~H. Bond. Geometric key quantization: reconstruction is not the
target. Companion Paper~II, 2026, doi:~10.5281/zenodo.21423312.
\item  A.~H. Bond. \emph{Geometric Observation: claim ledger,
prediction registry, and reproduction artifacts}. Companion repository, 2026, doi:~10.5281/zenodo.21423316.
\item  I.~Csisz\'ar. On an extremum problem of information theory.
\emph{Studia Sci.\ Math.\ Hungar.}, 9:57--71, 1974.
\item  A.~S. Kechris. \emph{Classical Descriptive Set Theory}.
Springer GTM 156, 1995 (Jankov--von Neumann uniformization, \S18).
\item  T.~Linder and R.~Zamir. High-resolution source coding for
non-difference distortion measures: the rate-distortion function. \emph{IEEE Trans.\
Inf.\ Theory}, 45(2):533--547, 1999.
\item  V.~Kostina and S.~Verd\'u. Fixed-length lossy compression
in the finite blocklength regime. \emph{IEEE Trans.\ Inf.\ Theory},
58(6):3309--3338, 2012.
\item  V.~N. Koshelev. Hierarchical coding of discrete sources.
\emph{Probl.\ Peredachi Inf.}, 16(3):31--49, 1980.
\item  M.~Effros. Distortion-rate bounds for fixed- and variable-rate
multiresolution source codes. \emph{IEEE Trans.\ Inf.\ Theory}, 45(6):1887--1910, 1999.
\item  J.~Nayak, E.~Tuncel, D.~G\"und\"uz, and E.~Erkip. Successive
refinement of vector sources under individual distortion criteria. \emph{IEEE Trans.\
Inf.\ Theory}, 56(4):1769--1781, 2010.
\item  G.~J. Op~'t~Veld and M.~C. Gastpar. Successive refinement of Gaussian
projections. EPFL preprint, 2017.
\item  W.~H.~R. Equitz and T.~M. Cover. Successive refinement of
information. \emph{IEEE Trans.\ Inf.\ Theory}, 37(2):269--275, 1991.
\item  B.~Rimoldi. Successive refinement of information: characterization of
the achievable rates. \emph{IEEE Trans.\ Inf.\ Theory}, 40(1):253--259, 1994.
\item  V.~Kostina and E.~Tuncel. Successive refinement of abstract
