...
bottleneck. We determine the complete two-stage rate region for two read operators---a
Gaussian source is successively refinable exactly when their optimal error covariances
nest---extend it to $k$ observers, and price a misidentified observer: mis-weighting a
read direction costs a bounded rate tax, omitting one erects an irreducible distortion
floor. Every empirical claim is sealed-registered against a preset bar; four of five
predictions are confirmed, one refuted, and a corrected trained-model test passed.
\end{abstract}
...
changes but the consumer; (vi) the complete two-stage rate region for two read
...
(\S\ref{sec:multiobs})---together with a finite-blocklength dispersion counting read
dimensions and an observer source--channel separation (\S\ref{sec:consequences}); and
(vii) an operational price for a \emph{mis}identified read operator: a bounded rate tax
for mis-weighting, an irreducible distortion floor for omission
(Appendix~\ref{app:mismatch}).
...
$Y=\PC^{1/2}X$ (Lemma~\ref{lem:tilt}). Hence the Kostina--Verd\'u [26]
...
Gaussian, cited from [26] \textup{[C7]}, not re-derived here.
\end{proof}
...
[25] non-difference regularity---we keep that hedge.
\end{remark}
...
boundary rates, and the operational region [32], [28], [33] lifts the
...
[33] (finite alphabets: [32]) lifts it. Refinability pins each
...
numerically (committed with the ledger [22]): the region converse survives
