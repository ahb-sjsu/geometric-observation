...
$\Sigma_x\ne I$. A prospective test on a trained Llama-3.2-3B layer (GO-P-2026-026) exhibits the floor: on
all $16$ KV heads the true read distortion $\tr(P\Sigma_\delta)$ saturates at the
\emph{measured} whitened-kernel value, and the infinite-rate misidentified coder plateaus
at a softmax divergence some $10^5\times$ that of the matched operator (median $0.10$
nats)---a genuine downstream floor. A finite-rate sweep initially missed this, the softmax
divergence still being dominated by the reducible read-mode error at the swept rates; the
floor is thus a high-resolution phenomenon whose finite-rate visibility scales with the
omitted direction's downstream weight. It remains a certificate about the read metric that
the consumer's own metric inherits only in the high-rate limit.
