...
further determine the complete two-stage rate region for two read operators
(\S\ref{sec:multiobs})---a Gaussian source is successively refinable exactly when their
...
(vi) the complete two-stage rate region for two read operators---with successive
refinability (the optimal error covariances nest) and a closed-form max-det rate loss as
its corner, extended to $k$ observers (\S\ref{sec:multiobs})---together with a
finite-blocklength dispersion counting read dimensions and an observer source--channel
separation (\S\ref{sec:consequences}); and (vii) an operational price for a
\emph{mis}identified read operator: a bounded rate tax for mis-weighting, an irreducible
distortion floor for omission (Appendix~\ref{app:mismatch}).
...
\begin{proposition}[Dispersion counts read dimensions]\label{prop:dispersion}
Fix $\PC$ of rank $r$ and an i.i.d.\ source with a finite-second-moment reference letter.
Under the excess- (or average-) distortion criterion the finite-blocklength consumer
coding problem for $d_{\mathcal C}$ is operationally identical, \emph{at every blocklength
$n$}, to the standard mean-squared-error problem for the $r$-dimensional tilted source
$Y=\PC^{1/2}X$ (Lemma~\ref{lem:tilt}). Hence the Kostina--Verd\'u \cite{kostinaverdu2012}
second-order expansion transfers verbatim,
\[
R_{\mathcal C}(n,\epsilon,D)=R_Y(D)+\sqrt{V_Y(D)/n}\,Q^{-1}(\epsilon)+\tfrac{\log n}{2n}
+o\!\left(\tfrac{\log n}{n}\right),
\]
with effective dimension $r_D:=\#\{\text{active water-filling modes}\}\le r$, equal to $r$
in the high-resolution limit $D\to0$---never the ambient $d$.
\end{proposition}
\begin{proof}
...
$2^{nR}$ points in $Y$-space, and both the excess-distortion event and the average
distortion of any codebook-mapping encoder depend on $Y^n$ and that codebook alone; so the
minimal codebook size is the $D$-covering number of $Y^n$, i.e.\ the $r$-dimensional MSE
problem. The encoder's side information on $\ker\PC$---genuinely correlated with $Y$ when
$\Sigma_x$ couples the blocks---is operationally inert, because covering $Y$ is insensitive
to extra data held only at the encoder. Thus $R_{\mathcal C}(n,\epsilon,D)=R_Y(n,\epsilon,
D)$ exactly at every $n$; the dispersion constants $V_Y$ are those of the $r$-dimensional
Gaussian, cited from \cite{kostinaverdu2012} \textup{[C7]}, not re-derived here.
\end{proof}
...
transmissible within consumer distortion $D$ iff $\rho\,R_{\mathcal C}(D)\le
C_{\mathrm{ch}}$; the strict inequality $\rho\,R_{\mathcal C}(D)<C_{\mathrm{ch}}$ suffices
and $\rho\,R_{\mathcal C}(D)>C_{\mathrm{ch}}$ is impossible.
\end{proposition}
\begin{proof}
...
converse and data processing across the memoryless channel. The only property used is that
$d_{\mathcal C}$ is an admissible distortion with a legitimate $R_{\mathcal C}$.
\end{proof}
...
positive definite and the distortion constraint binds. A two-stage code sends
...
unbounded-distortion instance we invoke the general-source extension
\cite{effros,kostinatuncel} under the reference letter $\hat x_0=0$ (which has
$\E d_i(X,0)^2<\infty$ for Gaussian $X$); strict convexity of the Gaussian $R_{P_i}$ on
the coding regime pins the per-letter distortions, so the operational two-stage corner
coincides with the single-letter region below.
...
If $X\to\hat X_2\to\hat X_1$ is Markov, then $\Sigma(\hat X_2)\preceq\Sigma(\hat X_1)$.
\end{lemma}
\begin{proof}
By the law of total covariance, refining the conditioning cannot increase the residual:
$\E[\Cov(X\mid\hat X_1,\hat X_2)]\preceq\E[\Cov(X\mid\hat X_1)]$; and under the Markov
chain $\Cov(X\mid\hat X_1,\hat X_2)=\Cov(X\mid\hat X_2)$ a.s.
\end{proof}
...
uses neither Markov monotonicity (Lemma~\ref{lem:markov}) nor uniqueness
(Lemma~\ref{lem:maxdet}).} \emph{Achievability.} Fix feasible $\Sigma_f\preceq\Sigma_b
...
boundary rates, and the operational region \cite{rimoldi,effros,kostinatuncel} lifts the
single-letter point to $\mathcal R$. Upward closure supplies the rest.
\end{proof}
...
For read operators $P_1,\dots,P_k\succeq0$ with targets $D_1,\dots,D_k$, the achievable
region is $\{(R_1,\dots,R_k):\exists\ \Sigma_k\preceq\cdots\preceq\Sigma_1\preceq
\Sigma_x,\ \tr(P_i\Sigma_i)\le D_i,\ \sum_{j\le i}R_j\ge\tfrac12\log\det\Sigma_x/\det
\Sigma_i\ \forall i\}$, and $(D_1,\dots,D_k)$ is refinable iff the $k$ single-stage optima
form a nested chain $\Sigma_k^\star\preceq\cdots\preceq\Sigma_1^\star\preceq\Sigma_x$.
\end{corollary}
\begin{proof}
...
i.i.d.), so $\Cov(X\mid Z_{1:i})=\Sigma_i$; the abstract-source region
\cite{kostinatuncel} (finite alphabets: \cite{rimoldi}) lifts it. Refinability pins each
...
chain (Cor.~\ref{cor:kobs}); we could not locate this general statement in the literature.
Open beyond it: non-Gaussian sources.
...
beyond the two-observer weighted-quadratic case that \S\ref{sec:multiobs} settles
(the general $k$-observer region, and non-Gaussian sources, remain open); and
...
P}(D/M)-R_P(D)\le\tfrac r2\log(M/m)$. The constant $r/2$ is exact---equality at $P=m\hat
P$ with only the loose $M$ known.
\end{theorem}
\begin{proof}
...
\emph{over}-covering probe ($\hat R\supseteq R$) has the same $o<1$ yet
$D_{\mathrm{floor}}=0$. What gates the floor is read energy lying \emph{outside} $\hat R$,
not the symmetric overlap.

\emph{The keys catastrophe, derived.} The boundary result of \S\ref{sec:boundary} (GO-5)
refutes the $\alpha=1$ density quotient for a direct inner-product consumer: the density is
\emph{read} (not in $\ker\PC$, Prop.~\ref{prop:rd}), so quotienting it removes a read
direction---an $\hat P$ whose range omits a direction the true $P$ reads, exactly
Theorem~\ref{thm:omission}'s hypothesis, hence a positive floor and irrecoverable failure
at any budget. The diffusion-domain case, where the density lies \emph{in} $\ker\PC$, is
...
projections. EPFL preprint, 2017.
