...
as $D\to0$), never the ambient $d$; the tilted-Gaussian dispersion $V_Y$ is cited---scalar
case \cite{kostinaverdu2012}, parallel/colored form \cite{ingberkochman}---not re-derived.
\end{corollary}
...
\emph{over}-covering probe ($\hat R\supsetneq R$, $\dim\hat R>r$) drives
$D_{\mathrm{floor}}$ to $0$ regardless of a below-one overlap. What gates the floor is the
fraction of $R$ lying \emph{outside} $\hat R$, not the symmetric overlap $o$.
