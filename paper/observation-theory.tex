...
condition on $\PC(x)$ this null distribution integrates to the fibers of
$\sim_{\mathcal{C}}$, and the consumer's true object is the quotient
$X/\!\sim_{\mathcal{C}}$, not $X$: perturbations in $\ker\PC$ exist in the
representation but have \emph{no operational consequence}. The consumer-lossless
floor $R_{\mathcal{C}}(0)=H(\Pi X)$ (Prop.~\ref{prop:floor}) is exactly the entropy of
that quotient---one pays only for what one can tell apart. Second, $\ker\PC$ is
...
declare failure and reconstruct with the constant $\hat x_0^{\,n}$. By the covering
lemma (the weak law applied to the i.i.d.\ information density and distortion) a
random codeword meets both conditions with probability $\ge2^{-n(I+2\varepsilon)}
(1-o(1))$, so $\Pr[\mathrm{fail}]\le\big(1-2^{-n(I+2\varepsilon)}(1-o(1))\big)^{2^{nR}}
\to0$ for $R>I+2\varepsilon$. On success the distortion is $\le D+\varepsilon$; on
