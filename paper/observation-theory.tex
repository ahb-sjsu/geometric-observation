...
\paragraph{A physical read operator (DOA).} The three read operators above are
\emph{planted}; the fourth is not, and its consumer is a physical estimator. In simulated
direction-of-arrival on a uniform linear array the consumer reads a source angle, and its
read operator is the array-manifold derivative orthogonalized against the steering vector,
$\hat g=\mathrm{normalize}(P_a^\perp\,a'(\theta_0))$, so $\PC=\hat g\hat g^\top$---\emph{not}
the signal subspace $\operatorname{span}(A)$, within which a subspace estimator is
scale-invariant, so error shaped there moves nothing (an initial $\operatorname{span}(A)$
design gave RMSE $0$ for every arm---the design pivot). On held-out parameters sealed before
the run (GO-P-2026-031), the estimator's own sensitivity recovers $\hat g$: the
finite-difference gradient of the angle estimate equals the analytic $\hat g$ at
$\cos=1.0000$. At matched log-precision the angle MSE orders invariant $<$ recon $<$ anti at
every rate ($6/6$, step ratios tracking $\kappa$) while the reconstruction-optimal arm
minimizes $\|x-\hat x\|$---the flip, on a physical estimator---and a shuffled read direction
produces no ordering. Two estimators sharing no algorithm (continuous beamformer-ML and
Phased-Array root-MUSIC) give the same $6/6$, their reconstruction-energy columns agreeing to
three significant figures. Omitting $\hat g$ floors the angle estimate at $1984\times$ the
rate-matched error---the Appendix-\ref{app:mismatch} omission, physically realized. Here the
read operator is \emph{recovered from a physical consumer}, not assumed; this is the
numerically-simulated companion to a pending X-band radar Gate~B (\texttt{[predicted]},
out-of-sample).
...
$\Sigma_x\ne I$. A sealed, held-out prospective test (GO-P-2026-027, layers $\{4,20\}$) confirms the floor
operationally on all $16$ KV heads: the read-metric floor is reached, and the infinite-rate
misidentified coder plateaus at a softmax divergence $800$--$5700\times$ that of the matched
operator (median $0.10$ nats)---a genuine downstream wall, out-of-sample from the layers on
which the effect was first seen. A finite-rate sweep initially missed it (the reducible
read-mode error dominates the swept rates), so the floor is a high-resolution phenomenon
whose finite-rate visibility scales with the omitted direction's downstream weight; the
consumer's own metric inherits it in the high-rate limit.
...
mode-budget inversion. Companion Paper~I, 2026, doi:~10.5281/zenodo.21423307.
\bibitem{paper2} A.~H. Bond. Geometric key quantization: reconstruction is not the
target. Companion Paper~II, 2026, doi:~10.5281/zenodo.21423311.
\bibitem{ledger2026} A.~H. Bond. \emph{Geometric Observation: claim ledger,
prediction registry, and reproduction artifacts}. Companion repository, 2026, doi:~10.5281/zenodo.21423315.
