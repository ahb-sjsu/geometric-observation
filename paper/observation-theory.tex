...
further prove an exact two-observer coding theorem (\S\ref{sec:multiobs}): a Gaussian
source is successively refinable across two distinct read operators if and only if
their optimal error covariances nest, with the excess rate given by a second max-det
program.
\end{abstract}
...
directions'') becomes a coding question with an exact answer, and a self-contained
theorem: the paper's read operator turned into rate.
...
\begin{theorem}[Two-observer refinability]\label{thm:multiobs}
Let $\Sigma_i^\star:=\Sigma^\star(P_i,D_i)$. The pair $(D_1,D_2)$ is successively
refinable if and only if the observers' optimal uncertainty ellipsoids nest,
$\Sigma_2^\star\preceq\Sigma_1^\star$.
\end{theorem}
\begin{proof}
\emph{Necessity.} Refinability supplies $(\hat X_1,\hat X_2)$ with $\E d_i\le D_i$,
$I(X;\hat X_1)\le R_1(D_1)$, $I(X;\hat X_1,\hat X_2)\le R_2(D_2)$. Since $\hat X_2$ alone
meets $D_2$, $I(X;\hat X_2)\ge R_2(D_2)$; with $I(X;\hat X_2)\le I(X;\hat X_1,\hat X_2)
\le R_2(D_2)$ this forces equality throughout, hence $I(X;\hat X_1\mid\hat X_2)=0$: the
chain $X\to\hat X_2\to\hat X_1$. Set $\Sigma_i:=\Sigma(\hat X_i)$. By Lemma~\ref{lem:cm},
$\tr(P_i\Sigma_i)\le\E d_i\le D_i$, so $\Sigma_i$ is feasible; by Lemma~\ref{lem:info},
$\tfrac12\log\det\Sigma_x-\tfrac12\log\det\Sigma_i\le I(X;\hat X_i)\le R_i(D_i)$, so
$\det\Sigma_i\ge\det\Sigma_i^\star$; by Lemma~\ref{lem:maxdet}, $\Sigma_i=\Sigma_i^\star$.
Lemma~\ref{lem:markov} then gives $\Sigma_2^\star\preceq\Sigma_1^\star$.
\emph{Sufficiency.} Let $\Lambda_i:=(\Sigma_i^\star)^{-1}-\Sigma_x^{-1}\succeq0$;
antitonicity of inversion turns $\Sigma_2^\star\preceq\Sigma_1^\star$ into
$\Lambda_1\preceq\Lambda_2$, so $\mathrm{range}(\Lambda_1)\subseteq\mathrm{range}
(\Lambda_2)$ and $K:=\Lambda_2^{+1/2}\Lambda_1\Lambda_2^{+1/2}$ satisfies $0\preceq K
\preceq I$ (extended by $0$ off $\mathrm{range}(\Lambda_2)$). With independent standard
Gaussians $N,N'$ set $Y_2:=\Lambda_2^{1/2}X+N$ and $Y_1:=K^{1/2}Y_2+(I-K)^{1/2}N'$; then
$X\to Y_2\to Y_1$, $Y_1$'s noise covariance is $K+(I-K)=I$ and its signal matrix
$K^{1/2}\Lambda_2^{1/2}$ contributes Fisher information $\Lambda_2^{1/2}K\Lambda_2^{1/2}
=\Lambda_1$, so $\Cov(X\mid Y_i)=(\Sigma_x^{-1}+\Lambda_i)^{-1}=\Sigma_i^\star$. With
$\hat X_i:=\E[X\mid Y_i]$, $\E d_i=\tr(P_i\Sigma_i^\star)\le D_i$, $I(X;\hat X_1)=
R_1(D_1)$, and $I(X;\hat X_1,\hat X_2)\le I(X;Y_1,Y_2)=I(X;Y_2)=R_2(D_2)$; the region of
\cite{rimoldi,effros,kostinatuncel} makes the corner $(R_1(D_1),R_2(D_2))$ achievable.
\end{proof}

\begin{theorem}[Exact rate loss]\label{thm:multiobs-loss}
Among two-stage codes whose base layer is stage-1-optimal ($R_1=R_1(D_1)$, $\E d_1\le
D_1$) and whose second stage meets $D_2$, the minimal total rate is $\tfrac12\log\det
\Sigma_x-\tfrac12\log\det\Sigma^\circ$, where $\Sigma^\circ:=\arg\max\{\log\det\Sigma:
\Sigma\preceq\Sigma_1^\star,\ \tr(P_2\Sigma)\le D_2\}$. Hence the rate loss is
$L(D_1,D_2)=\tfrac12\log(\det\Sigma_2^\star/\det\Sigma^\circ)\ge0$, with $L=0$ iff
$\Sigma_2^\star\preceq\Sigma_1^\star$.
\end{theorem}
\begin{proof}
\emph{Converse.} Any such code yields $(\hat X_1,\hat X_2)$ with $I(X;\hat X_1)\le
R_1(D_1)$, $\E d_1\le D_1$; as in Theorem~\ref{thm:multiobs}, $\Sigma(\hat X_1)=
\Sigma_1^\star$. The total rate is at least $I(X;\hat X_1,\hat X_2)\ge\tfrac12\log\det
\Sigma_x-\tfrac12\log\det\Sigma((\hat X_1,\hat X_2))$ (Lemma~\ref{lem:info} on the pair).
The pair covariance satisfies $\Sigma((\hat X_1,\hat X_2))\preceq\Sigma(\hat X_1)=
\Sigma_1^\star$ and $\tr(P_2\,\Sigma((\hat X_1,\hat X_2)))\le\E d_2\le D_2$
(Lemma~\ref{lem:cm}: the pair-conditional mean serves consumer~2 at least as well), so
it is feasible for the $\Sigma^\circ$ program. \emph{Achievability.} Run the
Theorem~\ref{thm:multiobs} construction with targets $(\Sigma_1^\star,\Sigma^\circ)$;
the nesting $\Sigma^\circ\preceq\Sigma_1^\star$ holds by constraint.
\end{proof}

Three consequences make the theorem legible. \emph{(i) Commuting observers.} If
...
We confirmed Theorems~\ref{thm:multiobs}--\ref{thm:multiobs-loss} numerically (committed
with the ledger \cite{ledger2026}): the nesting characterization, the Equitz--Cover
recovery, and the orthogonal-observer loss $L=R_1(D_1)$ all reproduce; sealed prereg
GO-P-2026-022, with a fresh-context derivation-grade pass logged.
...
phenomena---both are known---but their common generalization: two \emph{arbitrary,
possibly non-commuting, possibly different-kernel} positive-semidefinite read operators,
characterized by the single Loewner condition $\Sigma_2^\star\preceq\Sigma_1^\star$
(Thm.~\ref{thm:multiobs}) with the closed-form max-det rate loss
(Thm.~\ref{thm:multiobs-loss}); we could not locate this general statement in the
literature. Open beyond it: the general $k$-observer region and non-Gaussian sources.
