...
function. We derive the failure modes as the four degeneracies of $\PC$ and give the
central one an ex-ante \emph{sign} predictor: a read--energy alignment coefficient
$\kalign=\tr(\PCbar\Sx)/\sum_{i\le r}\lambda_i(\Sx)$ that near $1$ forbids the flip
before any code is built (the gradient-coupling null). We tested, and report as an honest
negative, the stronger claim that $\kalign$ reads off the flip \emph{magnitude}: it does
not --- the magnitude is confounded by read-subspace energy and the quantizer regime, so
$\kalign$ is a coupling-null predictor, not a continuous dial. Which code best exposes the flip is set not
...
At $\kalign=1$ (full coupling, D1) the read subspace is the top-energy subspace, reverse
water-filling already spends its budget on the read directions, and the flip vanishes:
$\Delta = d_{\mathcal C}(\mathrm{O}) - d_{\mathcal C}(\mathrm{R})\to0$. Away from
coupling, $\Delta$ can be positive.
\end{proposition}

\begin{remark}[what $\kalign$ does and does not predict --- honest negative]\label{rem:kappa}
The natural strengthening --- that $\Delta$ is \emph{monotone} decreasing in $\kalign$,
so that $\kalign$ reads off the flip magnitude as a single ex-ante number --- is
\textbf{not supported}. In honest simulation the flip magnitude is confounded by two
effects: (i) with the correct signal-scaled reconstruction-optimal quantizer, a
low-energy read subspace ($\kalign\to0$) carries little signal, so its absolute distortion
--- and $\Delta$ --- washes out, making $\Delta$ peaked in $\kalign$ rather than monotone;
and (ii) the flip magnitude depends on the quantizer-range regime (a shared/noise-floor
range, as in array direction-finding, versus a per-direction signal-scaled range, as in
embeddings). No tested normalization of $\Delta$ is monotone in $\kalign$. What survives
is the \emph{ordinal} prediction of Proposition~\ref{prop:kappa}: $\kalign\to1$ forbids
the flip (the gradient-coupling null), and the large-flip domains sit at small $\kalign$.
$\kalign$ is a coupling-null predictor, not a magnitude dial. We record this as an honest
negative (\texttt{results/GO-kappa-law.json}); a faithful magnitude law would require a
signal model matched to the quantizer regime and is left open.
\end{remark}
...
empirical null (flip $27\%$), the $\kalign\approx0.006$ array read is a large flip --- so
$\kalign$ is reported as a coupling-null predictor. The prospective magnitude seal
\seal{040} was \textbf{not} taken: the law it would have tested did not survive
validation. A magnitude law matched to the quantizer regime is left as open work.
