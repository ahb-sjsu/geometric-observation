...
histories is fresh, and a directional trigger beats the best isotropic
signal-aware policy by 16\% at matched update budgets --- the honest
baseline, since signal-aware sampling already beats age-based
...
on directional staleness beats the best isotropic signal-aware policy
at matched budgets (the falsifier in its honest form, since
signal-aware sampling already beats age-based scheduling
\cite{sun2020}). If long-horizon consequences dominate immediate
...
the consumer's read direction beats the best isotropic signal-aware
threshold by \textbf{16.3\%} pooled on the consumer endpoint, with the
...
directional trigger vs best isotropic signal-aware threshold at matched
