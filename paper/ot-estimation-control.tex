...
probing. First evidence exists: three campaigns --- blind
recovery~$\to$~scheduling with its analytic positive control, the
reconstruction-matched two-consumer flip in sensing, and signal-aware
operational triggering --- were sealed under the program's preregistration
protocol, run once each at governed seeds, and passed all gates
...
Three of the seven campaigns have been executed under the program's
preregistration protocol: design registered before any harness code existed;
...
preregistrations GO-P-2026-087/088/089, harnesses, and committed result
artifacts live in the program's campaign repository
...
the original sealing commits in \texttt{geometric-observation} remain the
binding registration timestamps.
...
experiment. Second, Campaign 2's governed invocation completed its
...
does. The first three governed campaigns passed exactly that test in its
synthetic instantiation (\S\ref{sec:evidence}), positive control included;
the physical endpoint is where the claim now goes to be risked.
