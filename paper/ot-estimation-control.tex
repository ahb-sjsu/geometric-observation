...
probing. First evidence exists: four campaigns --- blind
recovery~$\to$~scheduling with its analytic positive control, the
reconstruction-matched two-consumer flip in sensing, signal-aware
operational triggering, and a physical-model optical-link consumer probed
through the belief-averaged operator --- were sealed under the program's
preregistration protocol, run once each at governed seeds, and passed all
gates (\S\ref{sec:evidence}). OT is proposed not as a replacement for
...
Four of the seven campaigns have been executed under the program's
...
the realized error. The physical endpoint (Campaign 5) and the multi-consumer
and closed-loop questions (Campaigns 6--7) remain open.
...
does. The first four governed campaigns passed exactly that test ---
the fourth on a physics-based simulated optical link through the
belief-averaged operator (\S\ref{sec:evidence}), positive control
included; the hardware endpoint is where the claim now goes to be risked.
