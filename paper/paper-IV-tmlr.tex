...
(Methods); radar is data-limited; each domain uses a single model and dataset; the
clustered/hierarchical re-analysis (App.~\ref{app:domains}) and an independent replication
remain open. The sharpest open problem is \emph{magnitude, prospectively}: the blind
...
recordings; the honest analysis is a recording-clustered (hierarchical) bootstrap of the
paired difference $\Delta_i=L_i(\mathrm O)-L_i(\mathrm R)$, which we flag as the remaining
statistical work. The anti arm is a mechanistic control (deliberately non-exchangeable), so
we report its fraction rather than a worst-of-three chance test.
...
AV16.3 (034\,D2) & windows (201; 3 rec.) & 148/201 & recon-trade 201/201; \emph{cluster by rec.} \\
