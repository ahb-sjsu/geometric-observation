...
under-estimated). The prospective \emph{direction} is recoverable blind; the two-metric flip
and the point magnitude are not calibrated by an internal split alone, which is the honest
frontier (\S\ref{sec:discussion}).
...
remains open. The sharpest open problem is \emph{magnitude, prospectively}: the blind
non-oracle test (\seal{041}) recovered the winning code's \emph{direction} on untouched data
but missed the sealed magnitude band and lost the recon-trade on held-out. Predicting the
effect size --- not just its sign --- from $\tr[\PC(\Sd^{\mathrm O}-\Sd^{\mathrm R})]$ across
rate and domain, calibrated hierarchically, is the natural next contribution. And the $\kalign$ coordinate delivers less
