...
Minimizing over $U$ is maximizing $\tr(\Pi_{U}P_{i})$ over rank-$B$ orthogonal projections, and Ky Fan's maximum principle gives the value $\sum_{j\le B}\lambda_{j}(P_{i})$, attained exactly on top-$B$ eigenspaces.
...
Convexity is the time-sharing argument for a general distortion measure. If $(q_{1},\hat{x}_{1})$ and $(q_{2},\hat{x}_{2})$ meet distortions $D_{1},D_{2}$, the mixture channel that uses the first with probability $\lambda$ and the second otherwise, with the choice encoded in $W$, meets $\lambda D_{1}+(1-\lambda)D_{2}$, and mutual information is convex in the conditional law for a fixed source, so the mixture's rate is at most $\lambda I_{1}+(1-\lambda)I_{2}$.
The quadratic specialization is the definition, and the Gaussian statement is the change of variables $\tilde{x}=P^{1/2}x$ that turns the weighted quadratic distortion into squared error, after which the classical reverse water-filling applies on the covariance of $\tilde{x}$.
\end{proof}
...
The region is a definition.
What is proved about it is what the coding paper proved for its instance, and Section~\ref{sec:coding} states that instance.
...
\begin{proof}
For a $C^{2}$ family the Kullback--Leibler divergence between $\kappa(\cdot\mid y+\eta)$ and $\kappa(\cdot\mid y)$ equals $\tfrac12\eta^{\top}F(y)\eta+o(\|\eta\|^{2})$, which is the standard second-order expansion of the divergence with vanishing first-order term.
...
The coding paper studies an encoder that observes a jointly Gaussian pair $(Y,V)$, describes $Y$ at rate $R$ for a decoder that sees the description alone, while a third party retains a noisy copy $S=V+U$ of the context and never decodes.
Its conditional content is $L=I(X;\hat{X}\mid S)$ for the augmented source $X=(Y,V)$ with distortion on $Y$ alone.

\begin{corollary}[The coding region is a consumer-relative region]
\label{cor:coding}
Take $\M=\R^{2}$ with $\mu$ the law of $X=(Y,V)$, the consumer $C(y,v)=y$, the output metric $G=1$ on $\Y=\R$, and $S=V+U$ with $U$ independent Gaussian noise. Then $D_{C}(x,\hat{x})=(y-\hat{y})^{2}$, the read operator is the constant $\PC=\operatorname{diag}(1,0)$ whose kernel is the context direction, and the region \eqref{eq:region} is the rate--content region of the coding paper at distortion $D$ on $Y$, subject to that paper's operational constraints on the description and the reconstruction.
\end{corollary}

\begin{proof}
With $C$ the coordinate projection, $J_{C}=(1,0)$ and \eqref{eq:PC} gives $\operatorname{diag}(1,0)$ exactly, with no remainder in \eqref{eq:local} because $C$ is linear and $G$ is constant.
The distortion in \eqref{eq:region} is then $\E[(Y-\hat{Y})^{2}]\le D$, which is the coding paper's distortion constraint, and the two information inequalities are its rate and conditional content by definition.
\end{proof}

\begin{remark}
The corollary is an identification of definitions.
...
What the corollary adds is the reading. The kernel of the read operator is the context, the content $L$ is an information quantity about a variable the consumer never reads, and the non-determination result of that paper says that collapsing the source onto the consumer's quotient before encoding changes the operational problem.
That last point is the strongest evidence in the program that the quotient of Theorem~\ref{thm:rep}(a) is not the whole story once a third party retains context.
The operational constraints of the coding paper, in particular its common-reconstruction requirement, must be checked against Definition~\ref{def:region} before the corollary is cited as more than an identification of the distortion and the two information quantities.
\end{remark}

\section{Second Pushforward. Geometric Decision Cost}
\label{sec:decision}

The economics paper encodes each task as a reference point and a menu of alternatives in $\R^{9}$ and scores an alternative by the Mahalanobis distance of its displacement from the reference under a diagonal covariance $\Sigma$.
...
The economics paper reports that on the data of Andersen, Erta\c{c}, Gneezy, Hoffman, and List, collected in eight villages of Meghalaya in northeast India with 458 responders and stakes from 20 to 20,000 rupees, the largest exceeding the villagers' average yearly income, offers and rejections vary with the stake at every conventional level.
...
The reference implementation of the economics paper normalizes the monetary coordinate to the stake, so the encoding as released is stake-blind for every $G$, and a model that fits both stake ranges needs a monetary coordinate that carries the absolute stake together with a finite monetary variance.
...
The choice rule $P(a)\propto\exp(-c(a)/T)$ with a fixed cost is a Luce model with utility $-c/T$, it satisfies independence of irrelevant alternatives, and it satisfies regularity.
...
Every random-utility model satisfies regularity, because the event that $a$ has the largest utility in $A'$ is contained in the event that it has the largest utility in $A$.
...
Attraction and decoy effects in the marketing and psychology literatures are regularity violations of exactly this form, and the budgeted observer offers one mechanism for them.
...
The observation-theory program has an instrument, the blind probe, that recovers a consumer's read operator from calls to the consumer alone, and a theorem, the budget cliff, that says recovery at an operating point is a cliff at the full dimension of the space rather than a slope.
...
Overlap $\tr(\Pi_{\hat{U}_{k}}\Pi_{U_{0}})/k$, reported as resolution $(\text{overlap}-\text{chance})/(1-\text{chance})$ with chance $k/9$ for a random $k$-dimensional subspace, together with the rank certificate of the readscope instrument on $\hat{\bar{\Omega}}$.
...
The product correspondence $\beta=\prod_{i}\beta_{i}$ on the compact convex $S$ inherits these properties, and Kakutani's theorem gives a fixed point, which is a BGE.
This is the Debreu, Glicksberg, and Fan argument.
...
The BGE of the subclass are therefore the mixed Nash equilibria of the finite game, whose computation is PPAD-complete by Daskalakis, Goldberg, and Papadimitriou and by Chen, Deng, and Teng.
...
The first is the readscope instrument, already released, which recovers a read operator from consumer calls, certifies its rank, and reports the budget cliff.
The second is the eris-econ package, already released, which reproduces every number of the economics paper from the encodings of its Table IV.
...
The coding paper's region is the consumer-relative region for the projection consumer & identification of definitions, operational constraints to be checked & Corollary~\ref{cor:coding} \\
...
\section*{Sources}
The observer triple, the read operator, the quotient, the blind probe, and the budget cliff are defined in the Observation Theory encyclopedia and in the book chapters on the observer triple and on the read metric and the quotient, with Lean checks of the read-operator lemmas in the data-mining formalization.
The coding results are in ``Tradeoffs Between Rate and Conditional Content with Encoder-Observed Context.''
The decision results are in ``Geometric Prediction of Economic Behavior'' (IEEE Transactions on Computational Social Systems, accepted), whose Section IV and Table IV give the encoding used in Corollary~\ref{cor:decision}.
Regularity for random-utility models is Block and Marschak (1960) and Falmagne (1978).
Existence is Debreu (1952), Glicksberg (1952), and Fan (1952).
PPAD-completeness of Nash is Daskalakis, Goldberg, and Papadimitriou (2009) and Chen, Deng, and Teng (2009).
Ky Fan's maximum principle is Fan (1949).
Full bibliographic entries with DOIs are to be added before circulation.
