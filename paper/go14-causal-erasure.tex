...
\large GO-14 statement v0.7 --- Theorems R, C and T (the convex
program, and the $n$-transfer lemma), the (H$^\ast$) \emph{repair}
--- the cross-block read $X$, the lemma chain L1--L4, and the
split-causal sub-family --- T$^\ast$ v2, and \textbf{the plateau}: the
...
boundary charge from the plateau at $\Delta=0,1,2$}
\author{Geometric Observation program}
\date{v0.7 --- August 2026 (v0.6 superseded for the plateau:
...
the transfer route provably could not reach --- \emph{conditional on
(R1) alone}, the convexity of the process-rate functional on the
cyclostationary class, with (A)'s independent-noise-copy construction
as the other named hypothesis. Two scoping results are load-bearing and
...
$\Delta=0,1,2$ from \S\ref{sec:plateau}, conditional on (R1) alone;
...
\label{sec:plateau}
Status: spectral prover $+$ $\Psi$-bracket prover $+$ two independent
R-IND-5 passes (the bypass pass, ten restatements; the certificate
...
cell-argmin fixed point rather than the K1--K3 filter recursion). All
twenty restatements are in force in this revision. Harness
\texttt{experiments/\allowbreak go14\_plateau.py}; registration 082
pending --- \textbf{nothing in this section is sealed}.

\emph{The stationary frame.} Write $R_u:=\hat Y_{u-\Delta-1}$. In this
...
$5.3\times10^{-15}$; $\lambda_{\min}(R)\ge3.17\times10^{-3}$ on the
...
and never dropped in restatement.
\end{remark}

\subsection{Step (B): shift-averaging}

\begin{lemma}[(R1) --- convexity of the process-rate functional on the
cyclostationary class]
\label{lem:R1}
Let $F_T$ denote the total-rate functional on the window of length $T$
in process-moment coordinates. Then (i) $F_T$ is convex on the linear
section cut out by the cyclostationary class; (ii) on records whose
spectra are bounded above and bounded away from $0$, $F_T/T$ converges
\emph{pointwise} to the process rate; and (iii) a pointwise limit of
convex functions is convex. Hence the process-rate functional is convex
on the cyclostationary class.
\textbf{This lemma is stated here as a lemma, not cited as a corollary
of Theorem~\ref{thm:cvx}}: Theorem~\ref{thm:cvx} is a finite-window
statement and the passage to the process rate is exactly the content of
(ii)--(iii). \emph{Status: this is the single remaining load-bearing
hypothesis of the plateau} (together with
Hypothesis~\ref{hyp:indcopies}), and it is a
pointwise-limit-of-convex-functions statement --- a much softer object
than (H$^\ast$), (H$^{\ast\ast}$), (H$^{\ast\ast\ast}$), (D) or (F).
\emph{Numerics:} $69$ Jensen points and $40$ curvature lines on the
\emph{cyclostationary} class (the class the argument actually touches,
not merely the stationary one), zero violations.
\end{lemma}
...
\item[(iv)] of \emph{no larger rate}, by Lemma~\ref{lem:R1}:
...
\begin{corollary}[The chain, and its direction]
\label{cor:onedir}
...
\]
under Hypothesis~\ref{hyp:indcopies} and Lemma~\ref{lem:R1}.
...
plateau is \emph{not} hypothesis-free: it removes (H$^\ast$),
(H$^{\ast\ast}$), (H$^{\ast\ast\ast}$), (D), (F) and the boundary
charge --- a real and large advance --- and replaces them with
Lemma~\ref{lem:R1} and Hypothesis~\ref{hyp:indcopies}. It remains under
Hypothesis~\ref{hyp:uind} throughout.
\end{remark}
...
$R=P-Q\succeq0$}; both are load-bearing (R12). No matrix lift is
...
\label{cor:plateau2}
Under Hypothesis~\ref{hyp:uind}, Hypothesis~\ref{hyp:indcopies} and
Lemma~\ref{lem:R1} --- and \emph{conditional on
Lemma~\ref{lem:R1} alone} among the hypotheses this campaign has been
fighting ---
\[
...
(H$^{\ast\ast\ast}$), (D), (F) and the boundary charge from the plateau
argument, at three lags --- two of which the transfer route provably
could not reach} (it would have needed certified anchors at
$n\approx260$ and $n\approx1320$).
\end{corollary}
...
\begin{question}[Open]
\label{q:open}
(0) \textbf{Lemma~\ref{lem:R1}, and it is now the single remaining
load-bearing hypothesis of the plateau} (with
Hypothesis~\ref{hyp:indcopies}, which is a hypothesis of an explicit
construction rather than an estimate). It is a
pointwise-limit-of-convex-functions statement on the cyclostationary
class; a proof would leave Corollary~\ref{cor:plateau2} standing on
Hypothesis~\ref{hyp:uind} alone. Separately, the \emph{reverse}
inequality in Corollary~\ref{cor:onedir} --- a truncation /
achievability lemma at stationarity, which the non-rationality of the
stationary optimum makes non-trivial --- is open, and until it is
settled nothing may be said about $L^\infty$ beyond the bracket of
Remark~\ref{rem:onedirection};
...
Remark~\ref{rem:R7}, Lemma~\ref{lem:R1}, Theorem~\ref{thm:shift},
...
Corollary~\ref{cor:onedir} / Corollary~\ref{cor:plateau2}
\emph{conditional on Lemma~\ref{lem:R1}}. Nothing in
