...
$(R_b,S_b)$. In the within-block order fixed by
...
input.} Applied to the blocked process it gives the identity the whole
proof rests on,
\[
2\ln2\;n\;\mathrm{rate}
...
=-\ln\det(I-Z)$ with $Z=R^{1/2}h^\ast M_Q^{-1}hR^{1/2}$. The matrix
quadratic-over-linear map $(h,T)\mapsto h^\ast T^{-1}h$ is jointly
matrix-convex and Loewner-\emph{nonincreasing} in its denominator;
...
lift of Theorem~\ref{thm:cvx}(i).

\emph{(3b) (III) is convex --- the load-bearing new step.} Each frozen
...
\label{rem:R22}
Wiener--Masani needs only $\Phi\succeq0$ with $\ln\det\Phi\in L^1$; it
...
\item[(c)] the classical Wiener--Masani/Szeg\H o input, cited
...
\emph{Proof.} $\varphi(h,t)=hRh^\ast/t$ is jointly convex on $t>0$
(quadratic-over-linear, $R\succeq0$) and \emph{nonincreasing} in $t$;
...
convex, coordinatewise-monotone outer function gives $Z$ convex; and
$u\mapsto-\ln(1-u)$ is convex increasing on $u<1$.\hfill$\square$
...
Theorem~\ref{thm:cvx}(i)) is cited here only as the \emph{antecedent}
...
Remark~\ref{rem:onedirection}. Also owed, and \emph{not} a mathematical
gap: the \textbf{novelty sweep on the (R1) combination} --- blocked
...
Wiener--Masani/Helson--Lowdenslager and the 070/073 conditional-RDF
attributions. \textbf{No novelty language is used anywhere in this
document for that combination until the sweep is done};
...
combination is owed} and no novelty language for it appears anywhere
above.
\end{document}
