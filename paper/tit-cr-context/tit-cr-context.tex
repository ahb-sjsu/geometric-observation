...
augmented source $X=(Y,V)$ with distortion on $Y$ alone. Three results
are primary. First, a closed form:
...
index $M$ among codes meeting the distortion constraint. A
thermodynamic reading of this quantity, as ideal erasure work per
symbol, is stated once, as a hypothesis on the reset protocol rather
than a result, in Remark~\ref{rem:thermo}.
...
its hypothesis, and its scope. Task-based quantization
...
This remark states the paper's only physical reading. It is a reading
adopted under a stated hypothesis, not a result of this paper;
everywhere else $L_n$ and $L(D)$ are the coding quantities
\eqref{eq:work-def} and \eqref{eq:work-endpoint}. The reset model is the
ideal conditional-erasure protocol of \cite{delrio2011,faist2015},
specialized to classical side information: the third party may apply
ideal joint operations to the record and a work reservoir, conditioned
on the classical side information $S^n$, which is read without
disturbance and remains unchanged; only the record $M$ is required to
return to a standard state. For such protocols the single-shot results
of \cite{delrio2011,faist2015} bound the work of resetting $M$ above and
below by smooth conditional entropies of the pair $(M,S^n)$. Whenever
those smooth entropies agree with the Shannon conditional entropy
$H(M\mid S^n)$ up to $o(n)$, the minimal average work is
\begin{equation}
...
information. That agreement is an equipartition property of the pair
$(M,S^n)$, which a code does not make i.i.d.; the asymptotic statements
of \cite{delrio2011,faist2015} establish it for i.i.d.\ pairs, and we do
not prove it for the codes constructed in this paper. Under that
hypothesis, and only under it, Theorem~\ref{thm:main-region}, and
Theorem~\ref{thm:gaussop} for the Gaussian source, identify $L(D)$ with
the least asymptotic erasure work per symbol, in units of $\kb T\ln2$,
over codes meeting the distortion constraint. The scope of the reading:
...
Under the reading of Remark~\ref{rem:thermo}, adopted there as a
hypothesis on the reset protocol, $L(D)$ is the ideal erasure work per
symbol, in units of $\kb T\ln2$, paid by a reset mechanism that retains
$S$. The region states when storing
