...
decoder-side calculations; its $q=0$ conditional endpoint is Gray's
conditional function, which is the common-reconstruction value of
\cite{lu2016lossy} at $p_2=0$ (Appendix~\ref{app:lucompare}). An encoder restricted to $Y$ alone faces
...
\[
  ...
  W\leftrightarrow\hat Y,\quad (p_1,p_2)\leftrightarrow(p,q),
\]
their functional is identically the conditional-content functional of
Section~\ref{sec:binary}: $I(X,S_1;W\mid S_2)=I(Y,V;\hat Y\mid S)$ under the same
Markov chain $\hat Y-(Y,V)-S$ and the same Hamming constraint, which by pair
...
$h(p)-h(D)$ (their Remark~3), the $q=0$ anchor of Theorem~\ref{thm:binary}; at
$q=\tfrac12$ ($p_2=\tfrac12$) both reduce to the marginal rate--distortion
function $1-h(D)$; and at $D=0$ both give $h(p*q)$ (their Remark~1).
...
unconditional $R=I(Y,V;\hat Y)$, which their single-coordinate problem does not
