...
description alone. A third party retains a noisy copy of the context and
never decodes. We study the conditional content of the stored
description given that copy: the mutual information between source and
reproduction conditioned on the third party's side information,
Steinberg's common-reconstruction functional on the augmented source.
...
source--context correlation, and the noise in the third party's copy.
The region of jointly achievable rate and conditional content is
exact, with a Pareto frontier traced by two coupled water levels; rate
and conditional content strictly trade off exactly when the context is
partially informative, and the tradeoff persists when the copy is clean. The
conditional content is not determined by the joint law of the described
variable and the retained copy, so the encoder's access to the clean
context enters the function. The closed form recovers the classical,
Gray, and Steinberg formulas as limits. The rate--content frontier of the
doubly symmetric binary case is traced through a tilt equation, and coding
theorems, including a direct proof for the Gaussian source, give the
conditional content its operational meaning as the least normalized
conditional entropy of the stored index, the rate at which a party holding
the copy can re-encode it.
\end{abstract}
...
retains. That quantity matters when the description is eventually erased
by the party holding the side information: in the idealized erasure model
of Remark~\ref{rem:thermo}, the least average work of the erasure is
proportional to this conditional entropy, not to the entropy of the
description. The two quantities are set by different parties. The decoder
...
index $M$ among codes meeting the distortion constraint. That entropy
has a plain coding meaning. A reader that holds $S^n$ can store or forward
the index $M$ with a code whose expected length is within one bit of
$H(M\mid S^n)$ and no shorter, so $L_n$ is, to within $1/n$, the least
rate per source symbol at which the stored description can be re-encoded
for a party holding the noisy context. The decoder pays the rate $R_n$;
the party holding the context pays the conditional content $L_n$; and the
encoder, which alone sees the clean context, chooses a description that
serves one of them cheaply or balances the two. The thermodynamic reading
of the same quantity, as the infimum of the average erasure work per
symbol under an ideal conditional reset, is stated once, in
...
Pareto frontier is traced by a stationarity system in two coupled water
levels, one for each coordinate, and the two coordinates are minimized by
different test channels, so that rate and content strictly trade off,
...
this separation of the two optima misalignment. One consequence of the
...
The results depend on one another as follows. Theorem~\ref{thm:main-region} and
Lemma~\ref{lem:construction} give the discrete region and the
finite-blocklength construction; Lemma~\ref{thm:pair} reduces the
Gaussian problem to the pair; Lemma~\ref{lem:gauss} supplies the
Gaussian exhaustion; Theorem~\ref{thm:function}(a) is the closed form;
Proposition~\ref{prop:uniq} and Theorem~\ref{thm:function}(b) give
uniqueness and the region; Corollaries~\ref{cor:misalign}
and~\ref{cor:notmarginal} follow; and Theorems~\ref{thm:vector},
\ref{thm:binary}, and~\ref{thm:gaussop} are downstream.
...
two-water-level frontier (Theorem~\ref{thm:function}(b)) in place of
...
binary conditional-content function coincides with the
common-reconstruction rate of Lu \emph{et al.} \cite{lu2016lossy}, and
...
The closest common-reconstruction counterpart is Lu and Xu's vector
Gaussian Heegard--Berger problem with common reconstructions
\cite{luxu2021}. Their Theorem~4 gives the rate--distortion function
...
$P(g_R)=\rho^2\tau^2(1-D)(1-\rho^2)/\bigl(s(1-D-\rho^2)^2\bigr)>0$, so
...
holding the noisy context. Yamamoto priced distortion against source
...
cost of a conditional reset. Closer in form are the
...
\cite{sagawa2009,sagawa2012,parrondo2015}; the single-shot sharpenings
...
mutual information, and the two-water-level frontier is what it adds.
...
Section~\ref{sec:intro} retains $S^n$ and accesses the stored index $M$
in order to erase it; it never decodes, and $S^n$ is unchanged by the
erasure.
...
the third party retains the side information
...
to the retained side information, a name introduced here for the
normalized conditional entropy of the index; the thermodynamic reading of
Remark~\ref{rem:thermo}, after the coding theorem, is the reason for
studying it. Every statement and proof of the paper treats $L_n$ as
...
Lemma~\ref{lem:convexity} holds verbatim for standard Borel alphabets,
with the sums over $s$ replaced by integrals against the law of $S$ and
the conditional laws furnished by regular conditional probabilities: the
proof uses only the joint convexity of relative entropy
\cite{csiszar1967} and the linearity of the induced output law in the
channel. Theorem~\ref{thm:function}(b) and Theorem~\ref{thm:gaussop}
use this form.
\end{remark}
...
point is worth fixing at the outset: the reconstruction
...
(strongly) typical with $x^n$, declaring failure otherwise, and stores
...
and $\epsilon$, such that for all sufficiently large $n$ there is a
deterministic code
...
thermodynamics with side information: the third party, acting as the
reset mechanism, holds the record
...
heat bath at temperature $T$ (the temperature, not the pair of Section~\ref{sec:pair}), conditioned on $S^n$; the work supplied
may fluctuate from run to run and only its mean is counted; and only
$M$ is required to return to a standard state. The memory is the
ideal degenerate logical memory of Landauer's setting: its logical
states carry no energy difference, the protocol is cyclic in the
memory's Hamiltonian, and the side-information subsystem is left
unchanged; the identity below is the infimum in that model, not a claim
about a particular device. Under these conditions
...
approached, though not reached in finite time, by quasi-static
isothermal protocols conditioned on $S^n$ \cite{esposito2011,sagawa2009},
so the infimum of the
average work over protocols is
\begin{equation}
 W_n^{\star}\;=\;\kb T\ln2\,H(M\mid S^n)\;=\;n\,\kb T\ln2\,L_n ,
 \label{eq:landauer-bound}
\end{equation}
for every code. Hence, by Theorem~\ref{thm:main-region}, and by
Theorem~\ref{thm:gaussop} for the Gaussian source, $L(D)$ is the
infimum of the asymptotic average erasure work per symbol, in units of
$\kb T\ln2$, over codes meeting the distortion constraint. This is the average-work setting. The single-shot,
...
vector, and the unit vector with $Y=y_0^{\mathsf T}Z$, $Z=WT$ the whitened pair of
...
\begin{proof}
\emph{Step 1: the max-entropy bound.} The identity
$I(T;\hY\mid S)=h(T\mid S)-h(T\mid\hY,S)$ is used as an extended-real
identity: mutual information is the relative entropy of the joint law
with respect to the product of its marginals \cite{pinsker1964}, and
$h(T\mid\hY,S)=-\infty$ is the case of infinite conditional
information that Remark~\ref{rem:degenerate} treats. Write
...
the larger root, so $g_{\mathrm m}>g^\star$.
\end{proof}
...
the proof of Corollary~\ref{cor:misalign} later in this section).
...
in the proof of Theorem~\ref{thm:function}(b) gives attainment.
...
Theorem~\ref{thm:region}(b). The tradeoff is strict: the rate-optimal channel
...
The reason is worth stating plainly. Both instances have the same
...
$L$ of Theorem~\ref{thm:binary} in bits is this $L$ divided by $\ln2$:
...
(Proposition~\ref{rem:binfrontier}), with the Pareto arc from the tilt root
...
its two-water-level frontier and the misalignment condition of
...
binary. The closed forms were checked numerically against direct
minimization (Appendix~\ref{app:numver}).
...
establishes, in order: the converse by the chain rule;
closedness and right-continuity of the union; restriction to Gaussian
channels; and achievability in three steps, quantizing the side information
...
Every term is finite, being bounded by $H(M)\le\log|\mathcal M_n|$,
since $\hY^n$ is a function of the finite-valued $M$. The per-letter induced channels satisfy the
...
$\epsilon_n\to0$ for each fixed $(\epsilon,k,m)$. The limits below are
taken in this order: $\epsilon$ is fixed; $k$ is fixed by Step A; $m$ is
...
The author used a large language model assistant for drafting proofs and
prose sections, for editing, for the prior-art search summarized in
Section~\ref{sec:intro}, for writing the independent numerical check
scripts of Appendix~\ref{app:numver}, and for pre-submission review of the
manuscript. All proofs, claims, and numbers were checked by the author,
