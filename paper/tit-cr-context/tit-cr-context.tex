...
The reproduction notebook and check scripts of Appendix~\ref{app:numver}
are at \url{https://github.com/ahb-sjsu/geometric-observation}; the manuscript
sources and scripts are archived at doi:10.5281/zenodo.22678341.}}
...
between source and reproduction conditioned on it, Steinberg's
common-reconstruction functional on the augmented source.
The conditional content has a closed form for
jointly Gaussian sources, $L(D)=\tfrac12\log_2 g^\star$ with $g^\star$
the larger root of an explicit quadratic in the distortion level, the
source--context correlation, and the side-information noise.
The region of jointly achievable rate and conditional content is
exact, with a Pareto frontier traced by two coupled noise fractions; rate
and conditional content strictly trade off exactly when the context is
partially informative, and the tradeoff persists when the side information
is noiseless. The conditional content is not determined by the joint law
of the described variable and the side information, so the encoder's
access to the clean context enters the function. The closed form recovers the classical,
Gray, and Steinberg formulas as limits. The rate--content frontier of the
doubly symmetric binary case is computed explicitly, and coding theorems,
including a direct proof for the Gaussian source, give the conditional
content its operational meaning as the least normalized conditional
entropy of the stored index, the rate at which a party holding the side
information can re-encode it for its own storage.
\end{abstract}
...
the side information is $S=V+U$ with $U$ independent of $T$; the pair is
drawn from an ambient Gaussian vector $X$ of which $T$ is the relevant
part (Lemma~\ref{thm:pair}), and from Section~\ref{sec:pair} on $T$
itself is the source letter (Definition~\ref{def:problem}). The
operational theorem of this section needs no such structure. The encoder observes $X^n$
...
\begin{remark}\label{rem:specialization}
...
enters only through $T$, by Lemma~\ref{thm:pair}), and the distortion is
...
(normalized in Section~\ref{sec:pair}); $X$ is the source letter of the
discrete theorem and, in the Gaussian sections, the ambient vector from
which $T$ is drawn\\
...
is $\hY$, and once Lemma~\ref{thm:pair} below is proved, all
...
The author used Anthropic's Claude models, through the Claude Code
environment, to draft first versions of proofs and prose throughout the
manuscript, to edit, to run the prior-art search summarized in
Section~\ref{sec:intro}, to write the independent numerical check scripts
of Appendix~\ref{app:numver}, and for pre-submission review. All proofs,
claims, and numbers were checked by the author, who is responsible for
the content.
