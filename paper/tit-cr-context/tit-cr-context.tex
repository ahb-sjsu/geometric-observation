...
\url{https://github.com/ahb-sjsu/geometric-observation}; the manuscript sources and the verification scripts are archived at doi:10.5281/zenodo.22678341.}}
...
exact, with a Pareto frontier traced by two coupled water levels; the
two prices strictly trade off exactly when the context is partially
informative, and the tradeoff persists when the copy is clean. A structural consequence is that the
...
(Theorem~\ref{thm:region}(b) and Corollary~\ref{cor:misalign}). A
structural consequence of the closed form is non-determination by the
reduced marginal (Corollary~\ref{cor:notmarginal}): replacing the encoder's observation of
...
The remaining results are extensions, consistency anchors, and one
...
The dependency chain is short. Theorem~\ref{thm:main-region} and
...
priced coordinate is conditional on $S$, with which $V$ is correlated,
and access to $V$ strictly lowers the price for every $\rho\ne0$
...
The priced coordinate also has a security reading: $L$ is the
...
relevant pair (Lemma~\ref{thm:pair}). We call $S$
...
values are computed by the verification script. Fig.~\ref{fig:frontiermulti} repeats the construction at
...
a third party: the closed form of Theorem~\ref{thm:function}(a) for $Y$ and
...
channels; and achievability in three steps, quantizing the conditioner
...
construction), then returning to the unquantized conditioner through
...
footnote (\texttt{VERIFICATION.md} in the paper directory), together
with an executed notebook, \texttt{reproduce.ipynb}, submitted as
supplementary material, which evaluates every closed form against
direct minimization, regenerates the figures and tables, and runs the
three harnesses in one pass.
...
frontier (Proposition~\ref{rem:binfrontier}) have no counterpart in
