...
$R$ for a decoder that sees the description alone; $V$ is a correlated
variable, which we call the context. A third party holds a noisy copy $S=V+U$
of the context. Neither the decoder nor the third party decodes with $S$.
Instead $S$ enters through the conditional mutual information
$L=I(X;\hat{X}\mid S)$, which we call the conditional content of the
description: the minimal ideal work, in units of $k_BT\ln 2$, that a reset
mechanism retaining $S$ (the third party's role) must pay to erase the stored
description. We
characterize the minimal conditional content at distortion $D$. The function
has a closed form: $L(D)=\tfrac12\log_2 g^{*}$, where $g^{*}$ is the larger
root of an explicit quadratic in the correlation $\rho$ between the described
variable and the context and in the context noise $\tau^2$, in any ambient
dimension. It recovers the classical
...
context is misaligned with the described variable. We settle when the natural
determinant
...
is verified by independent numerical checks.
\end{abstract}
...
we characterize is none of them. When the described variable and the context
decouple ($\rho=0$), $L$ is Shannon's rate--distortion function. When the
third party's copy is clean ($\tau^2\to 0$), $L$ is Gray's conditional
rate--distortion function \cite{gray1973}. When the described variable and
the context merge ($\rho^2\to 1$), $L$ is
...
joint law whose values of $L$ differ (Section~\ref{sec:region}). The
encoder's access to the clean context $V$, which neither the decoder nor the
...
stationarity system, and shows the frontier is nondegenerate, so that rate
and content genuinely trade off, exactly when $\rho^2\in(0,1)$ and
$\tau^2>0$. Section~\ref{sec:floor} settles when the natural determinant
floor is attained, correcting a plausible conjecture: the attainment
condition involves the encoder-accessible conditional covariance, not the
third party's. Section~\ref{sec:vector} extends the frontier to vector
context. Section~\ref{sec:binary} closes the binary case completely: for a
...
$\hat{X}-X-S$, the same single-letter object as our $L(D)$, and
Steinberg solved its scalar Gaussian case. Lapidoth, Mal\"ar, and
Wigger \cite{lapidoth2014} relaxed exact to approximate reproduction
and solved the Gaussian problem with general jointly Gaussian side
information. In that entire line the encoder observes the source
alone, so the function is determined by the joint law of the source
and the side information. Our function is not: the encoder observes
the pair $(Y,V)$, and Section~\ref{sec:region} exhibits two
instances with the same $(Y,S)$ law and different $L$.
...
\cite{shlezinger2019}, semantic and goal-oriented communication
\cite{gunduz2023}, and the information bottleneck \cite{tishby1999}
also price descriptions by a downstream objective rather than by
reconstruction alone. The present problem is not a case of those
formulations, and we claim no containment in either direction; the
structural difference is the party that holds the conditioning
variable and the party that pays for it.
...
outputs the reproduction $\hXn$. The reset mechanism, which is the third
...
the description $M$ at rate $R_n$; the decoder reconstructs $\hXn$ from $M$
alone; the third party (the reset mechanism) holds the side information
$S^n$ alone and prices the erasure of $M$. In the Gaussian sections
$X=(Y,V)$ and $S=V+U$ with $U$ independent of $X$.}
\label{fig:system}
\end{figure}
...
$M$. The decoder reconstructs that codeword from $M$ alone. Standard
...
zero. This decoder is a proof device; the actual decoder of the code still
receives no side information. By Fano's inequality,
\begin{align}
 H(M\mid S^n)
...
\begin{figure}[!t]
\centering
\includegraphics[width=3.4in]{frontier}
...
\emph{work floor}\footnote{``Work'' refers to the ideal erasure work of
Section~\ref{sec:operational}: by \eqref{eq:landauer-bound}, a floor on the
conditional content $L$ is a floor on the reset work per symbol in units of
$\kb T\ln2$ \cite{landauer1961,bennett1973,faist2015}.}
\begin{equation}
...
with per-component equality, so $Z:=Y-\hat{\mathbf Y}$ has $\Cov Z=\Delta$,
...
\emph{The single-variable corollary.} For a single described variable, on
the active-floor
...
above). The Markov chain $\hat Y-(Y,V)-S$ means that one channel
$C=P(\hat y\mid y,v)$ is shared across the values of $S$. Then
$I(Y,V;\hat Y\mid S)=\sum_sP(s)\,I_s(C)$, where $I_s(C)$ is the mutual
information of the same channel $C$ under the input law
$P(Y,V\mid S=s)$. The input laws depend on $s$, but each $I_s$ is
convex in $C$ at its own fixed input law, so the nonnegative
combination is convex in $C$; the distortion is linear in $C$.
...
numerically by unconditional Lagrangian bounds, which assume no
convexity.
...
Corollary~\ref{cor:misalign}, the floor-attainment condition of
Theorem~\ref{thm:floor}, the vector-context extension of
...
rate--distortion function at $\rho=0$, to Gray's conditional function as
$\tau^2\to0$, and to Steinberg's common-reconstruction function as
$\rho^2\to1$. The structural moral is Corollary~\ref{cor:notmarginal}: the
function is set by what the encoder can see, not by the $(Y,S)$ joint law,
so conditional description content is a property of the encoding
...
description was not sized to, is studied operationally in companion
measurement papers \cite{bond2026landauer}. There the operational failure
rate of decisions made from stale or mismatched descriptions plays the role
that $L$ plays here as an information price. The misaligned regime
$\tau^2>0$ is exactly the regime those measurements probe; we claim no
equivalence beyond this correspondence of roles.
...
$2.5\times10^{-13}$\\
Reduction to the quadratic,
$(g{-}1)(D{-}h)-Q_1=(g{-}1)P(g)/(gk)$ & symbolic & exact\\
$L(D)=\tfrac12\log_2 g^\star$ vs direct minimization over $(a,b,n)$ &
6 random instances & max deviation $3.9\times10^{-16}$ bits\\
Uniqueness of the achieving channel & multistart optimization at
$\alpha=\tfrac12$ & spread $6.1\times10^{-9}$\\
Frontier of Theorem~\ref{thm:region} at
$\alpha\in\{0,\tfrac14,\tfrac12,\tfrac34,1\}$ & direct weighted
minimization & worst residual $3.7\times10^{-9}$\\
The six values of Corollary~\ref{cor:notmarginal} & exact surds vs
numerics & all four-decimal values confirmed\\
...
$3.4\times10^{-14}$\\
Vector-context system \eqref{eq:vecfoc} at $r=2$ & brute-force
minimization, five instances & values match; max residual
$3.0\times10^{-9}$\\
