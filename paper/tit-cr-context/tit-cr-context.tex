...
$L=I(X;\hat{X}\mid S)$ of the stored description relative to the retained
context: Steinberg's common-reconstruction functional applied to the
augmented source $X=(Y,V)$ with distortion on $Y$ alone. Under an ideal
conditional reset, $L$ equals the asymptotic erasure work per symbol in
units of $k_BT\ln 2$. Three results are primary. First, a closed form:
...
operational problem, and the augmentation is load-bearing. The closed form
...
Gaussian formula as $\rho^2\to1$. We further settle exactly when the
...
thermodynamic reading of this quantity, as minimal ideal erasure work
per symbol, is stated once, as Remark~\ref{rem:thermo}.
...
$(\rho,\tau^2)$; the known Gaussian solutions
\cite{steinberg2009,lapidoth2014} are scalar and unaugmented. Second, the
...
The remaining results present as structural consequences, extensions, and
consistency anchors, and the bracketed words in the theorem titles carry
the roles. Section~\ref{sec:floor} settles when the natural determinant
lower bound is attained, correcting a plausible conjecture: the
attainment condition involves the encoder-accessible conditional
covariance, not the third party's. Three extensions follow: the binary
...
is the evaluation of the reduced problem, and the closed form, the two
...
\cite{lu2016binary}; Steinberg \cite{steinberg2009}; Ahmadi et al.
\cite{ahmadi2013} & Binary Heegard--Berger with encoder side information
and common reconstruction \cite{lu2016binary}; decoder-SI CR values
\cite{steinberg2009,ahmadi2013} & Third-party conditioner; the joint
$(R,L)$ frontier and its tilt parametrization interpolating Gray at $q=0$
to the marginal RDF at $q=\tfrac12$; the conditional endpoint is compared
with \cite{lu2016binary} in Appendix~\ref{app:lucompare}\\
\hline
\end{tabular}
\end{table}
...
Section~\ref{sec:floor}. The closest common-reconstruction counterpart is
Lu and Xu's vector Gaussian Heegard--Berger problem with common
reconstructions \cite{luxu2021}, posed under positive diagonal
error-covariance constraints; its single-decoder, unpenalized-coordinate
limit is the natural point of contact with the conditioner-governed
attainment of Section~\ref{sec:floor}, a correspondence we note but do not
resolve here.
...
rate--distortion theory for secrecy systems \cite{schieler2014}. Two structural
...
\cite{delrio2011,faist2015}; Remark~\ref{rem:thermo} states the reading
and its scope, the paper's only physical claim. Task-based quantization
...
conditional endpoint of the binary case has a near neighbor in Lu
\emph{et al.} \cite{lu2016binary}, compared in Appendix~\ref{app:lucompare}. The present paper supersedes the
...
the side information is $S=V+U$ with $U$ independent of $X$; we call $S$
...
erasure role is in view (the two terms are synonyms throughout), retains
...
information. We name it the \emph{Landauer content} of the memory relative
to the retained side information; the term is ours, chosen for the
thermodynamic reading consolidated in Remark~\ref{rem:thermo} after the
coding theorem. Every statement and proof of the paper treats $L_n$ as
...
treated separately, in Appendix~\ref{app:gaussian}; no Gaussian result
of Sections~\ref{sec:pair}--\ref{sec:binary} relies on the
finite-alphabet theorem below.
...
is identified up to a bin of asymptotic size $2^{nI(X;\hX\mid S)}$; only
...
\eqref{eq:work-endpoint}. The reset model is the ideal
conditional-erasure protocol of \cite{delrio2011,faist2015}, specialized
to classical side information: the third party may apply ideal joint
operations to the record and a work reservoir, conditioned on the
classical side information $S^n$, which is read without disturbance and
remains unchanged; only the record $M$ is required to return to a
standard state. Under these protocols, the minimal average work to reset
$M$ while the third party retains $S^n$ is
\begin{equation}
 W_n^{\min}\;=\;\kb T\ln2\,H(M\mid S^n)+o(n)
 \;=\;n\,\kb T\ln2\,\bigl(L_n+o(1)\bigr),
 \label{eq:landauer-bound}
\end{equation}
Landauer's bound \cite{landauer1961} conditioned on the retained side
information, tight up to sublinear terms in the classical asymptotic
regime \cite{delrio2011,faist2015}. Hence, by
Theorem~\ref{thm:main-region}, and by Theorem~\ref{thm:gaussop} for the
Gaussian source, $L(D)$ is the least asymptotic erasure work per symbol,
in units of $\kb T\ln2$, over codes meeting the distortion constraint.
The scope of the claim: \eqref{eq:landauer-bound} prices only the
logically irreversible reset of the volatile index $M$; it does not
count the work of acquiring $X^n$, driving a channel, correcting errors,
or finishing in finite time \cite{landauer1961,bennett1973}, and the
codebook, wiring, and any permanent lookup tables are part of the
apparatus and are not erased in each cycle. Physical units are restored
by multiplying any content value in bits per symbol by $\kb T\ln2$.
\end{remark}

The content-only endpoint of the region is the minimum Landauer content, the
...
function, Remark~\ref{rem:cleanboundary} for the frontier, and
Lemma~\ref{lem:cleanctx} for the determinant bound; positivity
...
dispatched by Remark~\ref{rem:degenerate}. The determinant lower bound
of Section~\ref{sec:floor} can be negative at large distortions and is
quoted clipped, $[\Bdet]^{+}$. The closed form's root satisfies
...
establishes that a deterministic sufficient statistic $Z=\phi(T)$ with
$C-Z-T$ leaves the bottleneck tradeoff unchanged at every blocklength;
Theorem~\ref{thm:pair} is the case $Z=T=(Y,V)$, with the addition that the
...
(ii) As $\tau^2\to0$ at fixed $(\rho^2,D)$ with $D<1-\rho^2$, with
...
\begin{corollary}[Core: misalignment opens a strict tradeoff]
\label{cor:misalign}
...
$\RLs(D)=\tfrac12\log_2\bigl[(Q_0+n)/n\bigr]$ evaluated at the channel of
...
$(Q_1+n)/n=\bigl[(1-D)(1-\rho^2/s)+D\bigr]/D$, the displayed $\LRs(D)$.
...
$\RLs(D)=\tfrac12\log_2\bigl[(Q_0+n)/n\bigr]
...
\begin{corollary}[Core: $L$ is not determined by the reduced
marginal $(Y,S)$]
...
distortions; Table~\ref{tab:notmarginal} lists the values, in both cases
...
in bits. Exact forms for the second instance:
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{notmarginal}
\caption{Non-determination by the reduced marginal
(Corollary~\ref{cor:notmarginal}): $L(D)$ against $D$ for the two
instances $(\rho^2,\tau^2)=(\tfrac12,0)$ (dashed, squares) and
$(\tfrac34,\tfrac12)$ (solid, circles), which share the reduced
correlation $\operatorname{Corr}(Y,S)=\rho/\sqrt{s}=1/\sqrt2=0.7071$.
The two functions differ at every distortion shown; at the tabulated
levels (markers; Table~\ref{tab:notmarginal}) the gap is $0.0495$ bits
at $D=0.1$ and $0.1543$ bits at $D=0.3$.}
\label{fig:notmarginal}
\end{figure}
...
Fig.~\ref{fig:notmarginal} plots the two functions across distortions;
...
always.\footnote{In the thermodynamic reading of
Remark~\ref{rem:thermo}, the clipped form is the \emph{work floor} on
the reset work per symbol, $\kb T\ln2\,[\Bdet]^{+}$, the positive part
reflecting that the bound carries no information where the determinant
expression is negative.} The exactness
...
The determinant lower bound is attained, $L_{\min}=\Bdet$ (equivalently
$[\Bdet]^{+}$, since the conditions below force $\Bdet\ge0$), iff
\[
...
\paragraph*{Map of the attainment proof}
The proof tracks equalities through the chain \eqref{eq:floor} and reads
off one structural conclusion at each. The clean and noisy branches
split first: at $\tau^2=0$ the joint covariance of $(T,S)$ is singular
and the question reduces, by Lemma~\ref{lem:cleanctx}, to the
unconditional pair source with covariance $\Sigma_{Y\mid V}$. At
$\tau^2>0$ the chain is
$L\ge I(Y;\hat{\mathbf Y}\mid S)=h(Y\mid S)-h(Z\mid\hat{\mathbf Y},S)
\ge h(Y\mid S)-h(Z)\ge h(Y\mid S)-h(Z_A)-h(Z_B)\ge\Bdet$ with
$Z=Y-\hat{\mathbf Y}$, and attainment forces every inequality to
equality. Equality in conditioning-reduces-entropy makes $Z$
independent of $(\hat{\mathbf Y},S)$; the finiteness this step needs is
automatic along the chain, since $h(Z\mid\hat{\mathbf Y},S)=-\infty$
would make the conditional content infinite rather than equal to
$\Bdet$. Equality in subadditivity makes $Z_A\perp Z_B$. Equality in
the Gaussian maximum-entropy bound makes each $Z_i$ Gaussian with
variance exactly $D_i$, the distortion constraints active. Together
these pin the error law, $Z\sim\mathcal N(0,\Delta)$ independent of
$(\hat{\mathbf Y},S)$, and hence the moments
$\Cov(\hat{\mathbf Y})=\Sigma_Y-\Delta\succeq0$, the semidefinite part
of the conditions, and $\Cov(\hat{\mathbf Y},V)=\beta$. No Gaussianity
of the channel itself is used: the source $(V,Y,S)$ is Gaussian, so
$\E[V\mid Y,S]$ is linear, and the remaining equality
$I(T;\hat{\mathbf Y}\mid S)=I(Y;\hat{\mathbf Y}\mid S)$, conditional
independence of $V$ and $\hat{\mathbf Y}$ given $(Y,S)$, becomes a
vanishing partial covariance; the block inversion displayed in the
proof evaluates it as a positive multiple of
$\tau^2\,\Delta\Sigma_Y^{-1}\beta$, forcing $\tau^2=0$ or $\beta=0$ and
so producing the two branches (a) and (b). Throughout,
$\Delta\succ0$ and $\Sigma_T\succ0$ are in force (zero distortions and
degenerate covariances are excluded by the standing assumptions), and
the clipping is immaterial here: on the attainment branches
$\Bdet\ge0$, and where $\Bdet<0$ attainment fails trivially since
$L_{\min}\ge0$.
...
\begin{theorem}[Extension: the binary function]
\label{thm:binary}
...
coordinate $R=I(Y,V;\hat Y)$ is convex in the shared channel by the
...
The nearest published binary calculation is the binary Heegard--Berger
problem with encoder side information and common reconstruction of Lu,
Xu, Qian, and Wang \cite{lu2016binary}, in which the encoder observes
side information that may differ from the decoder's. Under the natural
relabeling of their source and side-information variables, that model
appears closely related to the conditional endpoint considered here;
Appendix~\ref{app:lucompare} fixes the relabeling and identifies which
endpoint results are inherited. Two further common-reconstruction
calculations are decoder-side-information ones. Steinberg's binary example
...
decoder-side calculations; whether its $q=0$ conditional endpoint
coincides with the value of \cite{lu2016binary} under the relabeling is
settled in Appendix~\ref{app:lucompare}. An encoder restricted to $Y$ alone faces
...
The thermodynamic reading is that of Remark~\ref{rem:thermo}: $L(D)$ is
the minimal ideal erasure work per symbol, in units of $\kb T\ln2$, paid
by a reset mechanism that retains $S$. The region states when storing
...
were not sized to, extends beyond source coding. In companion
...
variance of a conditional information density, and the quadratic cell
bound of Lemma~\ref{lem:cellbound} that controls its mean does not by
itself pin that variance, so a joint $(R_n,L_n)$ dispersion region does
...
Two devices are fixed first. The \emph{saturating dyadic quantizers}:
for $k\ge1$, let $q_k$ partition $\R$ into the bounded cells
$[j2^{-k},(j+1)2^{-k})$, $-4^k\le j<4^k$, together with the two
saturating tail cells $(-\infty,-2^k)$ and $[2^k,\infty)$; for a vector
conditioner $S\in\R^r$, apply $q_k$ to each coordinate, so the cells
are rectangles. The partitions are nested, every cell edge of $q_k$
being an edge of $q_{k+1}$, so the generated $\sigma$-fields
$\sigma(q_k(S))$ increase, and their union generates the Borel
$\sigma$-field of $S$, since the dyadic intervals do. Write
$S_k:=q_k(S)$. The second device is an integrability bound for the
quantized pair.

\begin{lemma}[Cell bound]\label{lem:cellbound}
Let $(\hY,S)$ be jointly Gaussian, $\hY$ scalar and centered, $S\in\R^r$,
with $\Sigma_{S\mid\hY}\succ0$, and fix $k$. For every cell $A$ of the
product quantizer $q_k$ there is a constant $C_A<\infty$ such that
\[
\Bigl|\,\ln\frac{\Pr(S\in A\mid\hY=\hat y)}{\Pr(S\in A)}\Bigr|
\;\le\;C_A\bigl(1+\hat y^{\,2}\bigr)
\qquad\text{for every }\hat y\in\R .
\]
Consequently the information density of the pair $(\hY,S_k)$ is bounded
in magnitude by a fixed quadratic in $\hat y$, and has finite moments
of all orders.
\end{lemma}

\begin{proof}
The conditional law of $S$ given $\hY=\hat y$ is Gaussian with mean
$m\hat y$ linear in $\hat y$ and fixed covariance
$\widetilde\Sigma:=\Sigma_{S\mid\hY}\succ0$; the marginal of $S$ is
Gaussian with covariance $\Sigma_S\succ0$.

\emph{Scalar $S$, bounded cells.} For a bounded cell $A=[u,v)$, the
ratio of the two integrals lies between the infimum and the supremum
over $A$ of the integrand ratio
$r(u',\hat y):=f_{S\mid\hY}(u'\mid\hat y)/f_S(u')$, so
$|\ln(\Pr(A\mid\hat y)/\Pr(A))|\le\sup_{u'\in A}|\ln r(u',\hat y)|$.
Here
\[
\ln r(u',\hat y)
=\ln\frac{\sigma_S}{\widetilde\sigma}
-\frac{(u'-m\hat y)^2}{2\widetilde\sigma^2}
+\frac{u'^2}{2\sigma_S^2},
\]
a quadratic in $(u',\hat y)$; taking the supremum over the bounded
range $u'\in[u,v)$ leaves a quadratic in $\hat y$ with coefficients
determined by the cell endpoints, which is the claimed bound.

\emph{Scalar $S$, tail cells.} For $A=[2^k,\infty)$,
$\Pr(A\mid\hat y)=\overline\Phi\bigl((2^k-m\hat y)/\widetilde\sigma\bigr)$
and $\Pr(A)=\overline\Phi(2^k/\sigma_S)$, a constant, where
$\overline\Phi$ is the standard Gaussian tail. The elementary bound
$|\ln\overline\Phi(z)|\le C(1+z^2)$ holds on all of $\R$: for
$z\le0$, $\overline\Phi(z)\in[\tfrac12,1)$; for $0<z\le1$,
$\overline\Phi(z)\in[\overline\Phi(1),\tfrac12)$; and for $z>1$ the
Mills-ratio bounds
$z\varphi(z)/(1+z^2)\le\overline\Phi(z)\le\varphi(z)/z$ give
$|\ln\overline\Phi(z)|\le z^2/2+\ln\sqrt{2\pi}+\ln(2z)$. With
$z=(2^k-m\hat y)/\widetilde\sigma$, $z^2\le2(4^k+m^2\hat
y^{\,2})/\widetilde\sigma^2$ is a quadratic in $\hat y$, and the
marginal term is a constant; the mirror cell is symmetric.

\emph{Vector $S$, rectangular cells.} For probabilities in $(0,1]$,
$|\ln(P/Q)|\le|\ln P|+|\ln Q|$, so it suffices to bound
$|\ln\Pr(S\in A\mid\hat y)|$ (the marginal term is a constant). The
rectangle $A$ contains the bounded rectangle $A'$ obtained by clipping
each infinite side to length one at its finite end, and
\[
\Pr(S\in A\mid\hat y)\;\ge\;\Pr(S\in A'\mid\hat y)
\;\ge\;\operatorname{vol}(A')\,
\frac{\exp\bigl(-\tfrac12\sup_{u\in A'}
(u-m\hat y)^{\mathsf T}\widetilde\Sigma^{-1}(u-m\hat y)\bigr)}
{(2\pi)^{r/2}\det(\widetilde\Sigma)^{1/2}},
\]
where the supremum over the bounded $A'$ is a quadratic in $\hat y$
(here $m$ is the vector regression coefficient). Since also
$\ln\Pr(S\in A\mid\hat y)\le0$, the claimed bound follows.

\emph{Consequence.} The information density of $(\hY,S_k)$ at
$(\hat y,j)$ is $\ln[\Pr(S_k=j\mid\hat y)/\Pr(S_k=j)]$; taking
$C:=\max_jC_{A_j}$ over the finitely many cells bounds it by
$C(1+\hat y^{\,2})$, which has finite moments of all orders under the
Gaussian law of $\hY$.
\end{proof}

In the application below, $\Sigma_{S\mid\hY}\succeq\Sigma_U\succ0$
automatically, because $S=V+U$ with $U$ independent of $(X,\hY)$.

\begin{theorem}[Extension: the Gaussian operational region]\label{thm:gaussop}
For the Gaussian source and every $0<D<1$,
\[
...
$|y_0-c|^2+n\le D'$, and coordinates
$\bigl(\tfrac12\log_2\tfrac{Q_0+n}{n},\
\tfrac12\log_2\tfrac{Q_1+n}{n}\bigr)$. The distortion constraint
confines $\theta$ to a compact set: $|y_0-c|^2\le D'$ is a closed ball
and $0\le n\le D'$; and feasibility forces $c\ne0$ there, since $c=0$
...
continuous where $n>0$ and diverging as $n\to0$ at $c\ne0$. Now let
...
gives $\tfrac12\log_2\bigl[(Q_0+n)/n\bigr](\theta^*)\le
\liminf_j\tfrac12\log_2\bigl[(Q_0+n)/n\bigr](\theta_j)\le R$, and
...
$D$, and fix $\epsilon>0$. Along the saturating dyadic quantizers,
\begin{equation}
...
achievability proof (covering, binning, and expurgation, as in the
discrete theorem) for this pair: a codebook of
...
conditions hold here: the $(X,\hY)$ density is a quadratic polynomial
in $(x,\hat y)$ for the jointly Gaussian pair, and the
$(\hY,S_\Delta)$ density is bounded by a fixed quadratic in $\hat y$,
with finite moments of all orders, by Lemma~\ref{lem:cellbound}.
Everything else,
...
repository. All checks pass. The load-bearing scalar algebra is
...
Table~\ref{tab:numver} summarizes the main checks.
...
$3.4\times10^{-14}$\\
Anchor convergence rates (Remark~\ref{rem:anchorrates}) & symbolic
implicit differentiation at the simple root; numeric gap ratios down to
$\epsilon=10^{-6}$ & coefficients exact; ratios within
$8.0\times10^{-4}$ of $1$\\
...
\label{app:lucompare}
% OWNER TODO: complete the formula-by-formula comparison after obtaining the
% closed-access primary source (Lu, Xu, Qian, Wang, WCSP 2016). This appendix is
% scaffolding: it fixes the relabeling and states what must be matched. Do not
% submit until the identification below is either confirmed or the endpoint
% novelty claim is withdrawn.

Lu, Xu, Qian, and Wang \cite{lu2016binary} solve a binary Heegard--Berger
problem with encoder side information and a common-reconstruction constraint.
Their setting is matched to the conditional endpoint of
Section~\ref{sec:binary} by the relabeling
\[
  X_{\mathrm{Lu}}\leftrightarrow Y,\qquad
  S_{\mathrm{enc}}\leftrightarrow V,\qquad
  S_{\mathrm{dec}}\leftrightarrow S ,
\]
under which their encoder-observed side information plays the role of the
encoder's context $V$ and their decoder side information plays the role of the
third party's copy $S$. At the conditional endpoint (content only) the present
functional is $\min I(T;\hY\mid S)$ under $\hY-T-S$ with Hamming distortion on
$Y$, the object their construction evaluates for the doubly symmetric binary
source, so under this relabeling their setting appears to reproduce the
conditional optimization here. The formula-by-formula identification of their
reported value with $L(D)$ at $q=0$, and the delineation of which endpoint
results are inherited, are to be completed against the primary source; the
conference article is closed access and the identification is not asserted until
it is checked. The joint $(R,L)$ frontier and the tilt parametrization of
Remark~\ref{rem:binfrontier}, which price both coordinates at once, are not to
our knowledge given by \cite{lu2016binary} and are the part of the binary
...
1997.\bibitem{villard2013} J. Villard and P. Piantanida, ``Secure multiterminal
source coding with side information at the eavesdropper,'' \emph{IEEE Trans.
Inf. Theory}, vol. 59, no. 6, pp. 3668--3692, 2013.\bibitem{schieler2014} C. Schieler and P. Cuff, ``Rate-distortion theory for
secrecy systems,'' \emph{IEEE Trans. Inf. Theory}, vol. 60, no. 12,
pp. 7584--7605, 2014.\bibitem{armstrong2026} J. Armstrong, ``Source-side sufficiency for the
