...
\cite{luxu2021}, stated for two decoders with side information and
diagonal positive definite constraints on every coordinate of the
reproduction, with achievability by the Timo--Chan--Grant scheme under
source enhancement. A constraint equal to the variance of the context
coordinate is slack, met by the zero reproduction, so the present
problem, one decoder and a constraint on the described variable alone,
is the specialization of their setting to a single decoder and a slack
context constraint. We have not verified that their expression reduces
to Theorem~\ref{thm:function} under that specialization, and we claim
the closed form as new only to that extent; both problems evaluate
Steinberg's functional, so the two must agree wherever both apply.
