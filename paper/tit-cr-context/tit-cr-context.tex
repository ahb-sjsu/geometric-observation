...
\url{https://github.com/ahb-sjsu/geometric-observation}; this manuscript, its full-length versions, and its verification scripts are archived at doi:10.5281/zenodo.22678341 (concept record; each version carries its own DOI).}}
...
never decodes. We characterize the conditional content of the stored
description relative to the retained context, the mutual information
between source and reproduction conditioned on the third party's copy.
This is Steinberg's common-reconstruction functional applied to the
augmented source, with distortion on the described variable alone. In
the average-work form of Landauer's bound with side information, the
conditional content is exactly the ideal erasure work per symbol. Three
results are primary. First, a closed form: the conditional content is
half the base-two logarithm of the larger root of an explicit quadratic
whose coefficients are set by the distortion level, the squared
correlation between the described variable and the context, and the
variance of the third party's noise, when the two observed variables are
scalar linear functionals of a jointly Gaussian source of any dimension.
Second, the exact rate--content region: its Pareto frontier is traced by
a coupled two-water-level stationarity system, and it is a nondegenerate
curve, so that rate and content strictly trade off, exactly when the
correlation is strictly between zero and one and the retained copy is
noisy, with the strict tradeoff persisting at the clean-context
boundary. Third, non-determination: two instances whose laws of the
described variable and the retained copy agree up to bijective scaling
can have different conditional content, so replacing the encoder's
observation of the pair by its reduced relation to the retained copy
changes the operational problem, and the augmentation is essential. The
closed form recovers the classical rate--distortion function when the
context is uncorrelated, Gray's conditional function when the retained
copy is clean, and Steinberg's scalar Gaussian formula when the
described variable and the context coincide. We also record exactly when
the natural determinant lower bound is attained, and give the joint
