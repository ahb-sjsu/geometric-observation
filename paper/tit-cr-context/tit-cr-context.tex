...
taken as the source letter, which Lemma~\ref{thm:pair} permits; the
...
establishes, in order: the converse with differential entropies;
...
$T=(Y,V)$ taken as the source letter $X$ (Lemma~\ref{thm:pair}), and
every $0<D<1$,
\[
...
The compactness step of the proof uses $D<1$. For $D\ge1$ the statement
follows from the zero reproduction, which meets the distortion
constraint with $R=L=0$, and $D=0$ is the extended-real limit with both
coordinates diverging. Throughout, $L_n$ is the coding quantity
...
I(X^n;\hY^n\mid S^n)$ are unchanged. The single-letterization holds with differential entropy $h$ in place of $H$: for the
...
Remark~\ref{rem:convgeneral}. Hence
