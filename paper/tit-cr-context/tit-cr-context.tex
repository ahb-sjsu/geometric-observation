% STATUS (M4, 2026-08-27): submission-ready draft; all proofs closed at full
% rigor and externally reviewed; framing = Steinberg functional on the
% augmented source (LMW Remark 8). History: EXTRACTION-PLAN.md status log.
% Verification: verify_converses.py 12/12; verifier_{sym,num}_checks.py 46+46;
% matlab_checks.m 11/11; lean/ObservationTheory/CRContext.lean zero sorry.
% OWED (owner): R-IND-5 fresh-prose pass; Steinberg-2009/Lu-2016 primary
% reads; Sec-VI split decision; submission-time sweep.
...
\markboth{IEEE Transactions on Information Theory (draft)}{Bond: Rate--Conditional-Content Tradeoffs with Encoder-Observed Context}
...
quadratic in $\rho$ and $\tau^2$, in any ambient dimension. The closed form recovers the classical rate--distortion
...
rate--distortion function. When the third party's copy is clean
...
Consequence\\
Theorem~\ref{thm:floor} & Determinant-bound exactness iff, governed by
$\Sigma_{Y\mid V}$ & Consequence\\
...
\end{equation}
so the bin rate exceeds $I(X;\hX)-I(S;\hX)$ by $\epsilon$, and the
Wyner--Ziv packing argument \cite{wynerziv1976} (the Markov lemma for
...
independent of $X$\\
$s=1+\tau^2$ & variance of $S$ after normalization\\
...
$L(1)-L(D)=0.0349$ bits.) When $\rho=0$ the classical reverse channel
...
Remark~\ref{rem:degenerate}). The lemma's positivity hypothesis
...
=\E\bigl[\Cov(\hat{\mathbf Y},V\mid Y,S)\bigr]
+\Cov\bigl(\E[\hat{\mathbf Y}\mid Y,S]-\hat{\mathbf
Y}_{\mathrm{lin}},\,0\bigr)=0 .
\]
...
%% Steinberg 2009 eq. (14) (binary example) confirmed via a citing
%% source (Vellambi-Timo, arXiv:1611.05467, Lemma 9 step (c));
%% primary read of the paywalled T-IT text still owed (house rule).
...
I(X^n;\hXn)$ and $H(M\mid S^n)\ge I(X^n;M\mid S^n)\ge
I(X^n;\hXn\mid S^n)$ are unchanged. The single-letterization of
...
$H(M)\le\log|\mathcal M_n|<\infty$, since $\hXn$ is a function of the
...
lower semicontinuity of mutual information \cite{posner1975}).
...
repository harness \texttt{verify\_go11\_m2sys\_binary.py}. All scripts and
