...
% STATUS: M2 converse closure (2026-08-26) + external-review mathematical
% repairs (2026-08-27): misalignment iff rescoped to the standing model with
% the tau=0 persistence remark; floor section repaired (clean-context branch
% via conditional-support reduction, necessity expanded, determinant bound
% renamed with clipping, Chen et al. 2026 cited); Gaussian operational theorem
% proved in Appendix (converse full; achievability via quantized conditioner
% with two imported classical ingredients); binary tilt-root
% existence/uniqueness lemma added. All checked by verify_converses.py.
% Repositioning round (2026-08-27) done: title/abstract/intro reframed to the
% augmented-source reading (LMW Remark 8 verified), IB identity added, prior
% -manuscript delineation added; all flagged sentences reworded.
% The R-IND-5 fresh-context pass on new prose remains OWED (house rule).
...
and neither the decoder nor the third party decodes with $S$. Instead $S$
enters through the conditional mutual information $L=I(X;\hat{X}\mid S)$,
the conditional content of the description: the minimal ideal work, in units
of $k_BT\ln 2$, to erase the stored description by a reset mechanism
retaining $S$. This is Steinberg's common-reconstruction functional applied
to the augmented source $X=(Y,V)$ with distortion on $Y$ alone. We give it
this operational meaning and evaluate it in closed form:
$L(D)=\tfrac12\log_2 g^{*}$, with $g^{*}$ the larger root of an explicit
quadratic in the correlation $\rho$ and the context noise $\tau^2$, in any
ambient dimension. The closed form recovers the classical rate--distortion
...
context channel the function closes completely, with an elementary proof,
through a tilt equation anchored at Gray's function and the marginal
...
The main results are these. Section~\ref{sec:pair} reduces the problem in
any ambient dimension to the pair $(\rho,\tau^2)$. Section~\ref{sec:function}
...
Section~\ref{sec:region} characterizes the exact region of achievable
rate--content pairs, whose Pareto frontier is traced by a two-water-level
stationarity system, and shows that within the strictly noisy model
$\tau^2>0$ the frontier is nondegenerate, so that rate and content genuinely
trade off, exactly when $0<\rho^2<1$; the tradeoff persists at the
clean-context boundary $\tau^2=0$. Section~\ref{sec:floor} settles when the
...
conditional covariance, not the third party's. Section~\ref{sec:vector}
extends the frontier to vector context. Section~\ref{sec:binary} closes the
binary case completely: for a doubly symmetric binary source with a binary
symmetric context channel, an elementary and self-contained proof yields the
function through a tilt equation, anchored at Gray's binary conditional
function at one end and the marginal rate--distortion function at the other.
Appendix~\ref{app:gaussian} proves the operational region theorem directly
...
distortion on $Y$ alone. What we add is the conditional-erasure
operational reading, the closed-form evaluation of the augmented
Gaussian and binary cases, the joint tradeoff with description rate,
and the observation (Section~\ref{sec:region}) that $L$ is not
determined by the reduced marginal $(Y,S)$: the augmentation is
load-bearing.
...
\begin{theorem}[Determinant-bound exactness]\label{thm:floor}
...
\begin{theorem}[Gaussian operational region]\label{thm:gaussop}
For the Gaussian source,
\[
