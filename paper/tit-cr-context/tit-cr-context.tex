...
reproduction conditioned on the third party's side information, which is
Steinberg's common-reconstruction functional on the augmented source with
distortion on the described variable alone. Two results are primary. The
conditional content has a closed form for jointly Gaussian sources of any
dimension, $L(D)=\tfrac12\log_2 g^\star$ with $g^\star$ the larger root
of an explicit quadratic in the distortion level, the source--context
correlation, and the noise in the third party's copy. And the region of
jointly achievable rate and conditional content is exact, with a Pareto
frontier traced by two coupled water levels; the two prices strictly
trade off exactly when the context is informative but not perfectly so
and the third party's copy is noisy, and the tradeoff persists as that
noise vanishes. A structural consequence is that the conditional content
is not determined by the joint law of the described variable and the
retained copy: the encoder's access to the clean context enters the
function, so the augmentation is essential. The closed form recovers the
classical rate--distortion function, Gray's conditional function, and
Steinberg's scalar Gaussian formula at its three corners. The doubly
symmetric binary case is solved through a tilt equation. Coding theorems,
...
entropy of the stored index given the third party's side information.
\end{abstract}
...
(Theorem~\ref{thm:function}), after Lemma~\ref{thm:pair} reduces the
...
(Theorem~\ref{thm:region} and Corollary~\ref{cor:misalign}). A
...
The remaining results are extensions and consistency anchors.
Remark~\ref{rem:floor} reads off the closed form when the determinant
lower bound is attained: the condition involves the encoder-accessible
...
reading, its setting, and its scope. A non-common-reconstruction baseline is remote source coding with
...
\emph{(b) The region.} The closure of the single-letter rate--content pairs
...
The closed form settles when the conditional determinant lower bound is
...
Corollary~\ref{cor:misalign}, the attainment condition for the determinant bound
