...
\title{Rate--Conditional-Entropy Tradeoffs\\ with Encoder-Observed Context}
...
The manuscript sources, the reproduction notebook, and the check scripts of
Appendix~\ref{app:numver} are at
\url{https://github.com/ahb-sjsu/geometric-observation/tree/tit-cr-context-1.7/paper/tit-cr-context}
(tag \texttt{tit-cr-context-1.7}) and are archived at
doi:10.5281/zenodo.22678341.}}

\markboth{IEEE Transactions on Information Theory}{Bond: Rate--Conditional-Entropy Tradeoffs with Encoder-Observed Context}

\maketitle

\begin{abstract}
An encoder observes a jointly Gaussian pair, a described variable and a
correlated context, and describes the first for a decoder that sees the
description alone. A third party holds side information, a noisy copy of
the context, and never decodes. We study the conditional content of the
stored description given that side information, the mutual information
between source and reproduction conditioned on it, which is Steinberg's
common-reconstruction functional on the augmented source. For jointly
Gaussian sources it has a closed form, $L(D)=\tfrac12\log_2 g^\star$
with $g^\star$ the larger root of an explicit quadratic in the distortion
level, the source--context correlation, and the side-information noise.
The region of jointly achievable rate and conditional content is exact,
with a Pareto frontier traced by two coupled noise fractions. Rate and
conditional content strictly trade off exactly when the context is
partially informative, and the tradeoff persists when the side
information is noiseless. The conditional content is not determined by
the joint law of the described variable and the side information, so the
encoder's access to the clean context enters the function. The closed
form recovers the classical, Gray, and Steinberg formulas as limits. The
rate--content frontier of the doubly symmetric binary case is computed
explicitly, and coding theorems, including a direct proof for the
Gaussian source, give the conditional content its operational meaning as
the least normalized conditional entropy of the stored index, the rate at
which a party holding the side information can re-encode it.
\end{abstract}

\begin{IEEEkeywords}
Common reconstruction, conditional mutual information, Gaussian sources,
...
A stored description of data can be measured in two ways. The first is
its rate, the space the description occupies, which rate--distortion
theory trades against the fidelity the decoder requires. The second is
its conditional entropy given side information that some other party
retains: a reader holding that side information can store or forward the
description with a prefix-free code whose expected length is within one
bit of the conditional entropy and no shorter, so the conditional entropy
is the least rate at which the description can be re-encoded for that
reader. The two quantities are set by different parties. The decoder
never sees the side information, and the party holding it never serves
the decoder. This paper solves the source coding problem that this
asymmetry creates: in closed form for Gaussian sources, and, for the
doubly symmetric binary source, by tracing the rate--content frontier
through a tilt equation.

A concrete instance is a two-tier data store (Fig.~\ref{fig:system}). A
serving tier keeps the compressed records and answers queries from the
descriptions alone; it pays the rate. An archival tier keeps a copy of
every description together with metadata correlated with the records,
which the encoder saw exactly and the archive holds only approximately,
and it stores each description compressed against that metadata; it
pays the conditional content. One encoder, which sees the record and
its exact context, produces one description for both tiers, and the
question is how cheap it can make the description for one tier without
making it expensive for the other.
...
\node[lbl] at (2.7,-0.78) {sees $T^n$ only};
\node[lbl] at (6.7,-0.78) {sees $M$ only};
\node[lbl] at (6.7,-2.88) {holds $M$, retains $S^n$, does not
reconstruct $\hY^n$};
\end{tikzpicture}
\caption{The system. The encoder observes the source alone and stores
the description $M$ at rate $R_n$; the decoder reconstructs from $M$ alone;
the third party retains the side information $S^n$ and holds the stored
index $M$; it never reconstructs. The figure shows the Gaussian
...
index $M$ among codes meeting the distortion constraint. The decoder pays
the rate $R_n$; the party holding the side information pays the
conditional content $L_n$; and the encoder, which alone sees the clean
context, chooses a description that serves one of them cheaply or
balances the two. The same quantity has a thermodynamic reading, as the
infimum of the average work of erasing the index in an idealized
conditional reset; it is stated once, in Remark~\ref{rem:thermo}, with
the assumptions it needs, and nothing else in the paper depends on it.
...
solution of the augmented problem, and two contributions are primary.
...
(Theorem~\ref{thm:function}(a)), after Lemma~\ref{thm:pair} reduces the
problem, for $Y$ and $V$ scalar linear functionals of an
arbitrary-dimensional jointly Gaussian source, to the pair
$(\rho,\tau^2)$. The known Gaussian evaluations do not cover this one.
Steinberg's Gaussian example \cite{steinberg2009} and the Gaussian
theorem of \cite{lapidoth2014} take a scalar source with side
information $X+U$ and an encoder that observes the source alone; the
augmentation remark of \cite{lapidoth2014} is stated for finite
alphabets; and the vector Gaussian common-reconstruction function of Lu
and Xu \cite{luxu2021} covers the augmented source only with the context
penalized, under a hypothesis that excludes the unpenalized limit
evaluated here (Appendix~\ref{app:lucompare}). Second, the
exact region of jointly achievable rate and conditional content. Its
Pareto frontier is traced by a stationarity system in two coupled noise
fractions, one for each coordinate, and the two coordinates are minimized
by different test channels, so that rate and content strictly trade off,
exactly when $0<\rho^2<1$ within the noisy model $\tau^2>0$; the
tradeoff persists at the clean-context boundary
(Theorem~\ref{thm:region}(b) and Corollary~\ref{cor:misalign}). We call
this separation of the two optima misalignment. Within the noisy model
the endpoint excesses are small, at most $0.11$ bits of rate and $0.08$
bits of content over a broad parameter box, and the rate excess is
unbounded only toward the clean-context boundary
(Section~\ref{sec:region}). One consequence of the
closed form is that $L(D)$ is not determined by the joint law of $(Y,S)$
(Corollary~\ref{cor:notmarginal}): two sources whose $(Y,S)$ laws agree up
to invertible scaling can have different minimal conditional content.
This is why the function is not Steinberg's scalar formula: the encoder's
access to the clean context $V$, which neither the decoder nor the third
party shares, enters the function itself. The coding theorems support
these results: they characterize $L(D)$ as the least asymptotically
achievable normalized conditional entropy $H(M\mid S^n)/n$ of the stored
index (Section~\ref{sec:operational}, and Appendix~\ref{app:gaussian} for
the Gaussian source), and the thermodynamic reading of $L(D)$
(Remark~\ref{rem:thermo}) is one consequence of that characterization.

The remaining results are extensions, classical limits, and one
attainment remark. Remark~\ref{rem:floor} reads off the closed form when
the conditional-variance lower bound is attained. Three extensions
follow: the rate--content frontier of a doubly symmetric binary source
with a binary symmetric context channel is traced through an elementary
tilt equation
...
(Appendix~\ref{app:gaussian}). The classical limits are the three
corners of the closed form, Shannon's rate--distortion function
...
suboptimality of marginalization (Corollary~\ref{cor:margstrict}) and an
exact information-bottleneck identity for the weighted objective.

\subsection*{Related work}

Conditional rate--distortion theory is due to Gray
\cite{gray1972tr,gray1973}: both terminals observe the conditioning
variable, and $R_{Y\mid S}(D)$ prices the description under shared
context. Here no terminal observes $S$, and Gray's function appears only
as the clean-context limit $\tau^2\to0$ (Section~\ref{sec:function}).
Wyner and Ziv \cite{wynerziv1976} place the side information at the
decoder, where it participates in reconstruction; our decoder holds
nothing but the description, and Wyner--Ziv binning enters the
operational theorem only as a proof device.

Steinberg \cite{steinberg2009} introduced the common-reconstruction
constraint, under which the decoder holds side information but the
encoder must be able to reproduce the decoder's output; the resulting
functional $\min I(X;\hX\mid S)$ under $\hX-X-S$ is the single-letter
object of this paper, and Steinberg solved its scalar Gaussian case.
Lapidoth, Mal\"ar, and Wigger \cite{lapidoth2014} relaxed exact to
approximate reproduction and solved the scalar Gaussian problem with
side information $X+U$; their Remark~8, stated for finite alphabets,
incorporates an encoder observing an additional correlated sequence by
augmenting the source and letting the distortion ignore the extra
coordinate. Our problem is exactly that augmented problem, with
$X=(Y,V)$ and distortion on $Y$ alone. What is inherited is the
functional and the augmentation device; what is new, to our knowledge,
is the evaluation: the closed form and achieving channel of
Theorem~\ref{thm:function}(a) for $Y$ and $V$ scalar linear functionals
of an arbitrary-dimensional source, the joint region with its
two-noise-fraction frontier (Theorem~\ref{thm:function}(b)) in place of
the rate under a common-reconstruction constraint alone, the
misalignment condition (Corollary~\ref{cor:misalign}), and the
non-determination corollary (Corollary~\ref{cor:notmarginal}). The
encoder-side-information common-reconstruction model of Lu \emph{et al.}
\cite{lu2016lossy,lu2016binary} is this paper's formulation for the
binary source: under the relabeling $X\leftrightarrow Y$,
$S_1\leftrightarrow V$, $S_2\leftrightarrow S$, $W\leftrightarrow\hY$
their functional is the binary conditional-content function. Their
conditioned quantity is a transmitted rate with a Wyner--Ziv saving,
while here the transmitted rate is the unconditional $R$ and the
conditioned quantity is a separate cost, so their single-coordinate
problem does not constrain $R$; the binary contribution here is the
joint frontier (Appendix~\ref{app:lucompare}).

Side information held by the encoder alone is classically inert: when
it does not enter the distortion measure, it leaves the rate--distortion
function unchanged \cite{berger1971}, and Kaspi \cite{kaspi1994}
analyzed the configurations in which the encoder is informed of the
side information. Here the encoder-held context $V$ matters because the
second coordinate is conditional on $S$, with which $V$ is correlated,
and access to $V$ strictly lowers it for every $\rho\ne0$
(Corollary~\ref{cor:margstrict} for $\tau^2>0$;
Proposition~\ref{prop:marg} at the clean-context boundary). Heegard and Berger
\cite{heegardberger1985} serve decoders with and without side
information from one description, and Ahmadi, Tandon, Simeone, and
Poor \cite{ahmadi2013} imposed the common-reconstruction constraint on
the Heegard--Berger and cascade problems; Section~\ref{sec:binary}
distinguishes the binary frontier from their calculations.

The closest Gaussian counterpart is Lu and Xu's vector Gaussian
Heegard--Berger problem with common reconstructions \cite{luxu2021},
whose Theorem~4 covers the augmented source $(Y,V)$ with the context
penalized by its own distortion constraint $D_V$, under a hypothesis
that admits no $D_V$ beyond $1-\rho^2/(1-D)$. The present problem is the
unpenalized limit $D_V\to\infty$ of that constraint family, outside the
hypothesis; Appendix~\ref{app:lucompare} shows that their closed form
lies strictly above $L(D)$ on its admissible range and does not continue
to the limit.


The conditional coordinate also has a security reading: $L$ is the
equivocation $H(M\mid S^n)/n$ of the stored description at a party
holding the side information. Yamamoto priced distortion against source
equivocation in the Shannon cipher system \cite{yamamoto1997} and, in an
earlier problem \cite{yamamoto1983}, let the encoder observe a source
together with an additional output whose equivocation at the receiver
or wiretapper is to stay high; Villard and Piantanida \cite{villard2013}
and Schieler and Cuff \cite{schieler2014} developed secure source coding
with side information at the eavesdropper and a rate--distortion theory
for secrecy systems. That line prices the equivocation of the source and
seeks to make it large, and its encoder does not observe the clean
context; here the equivocation of the description is made small, as the
re-encoding cost at the party holding the side information. Closer in
form are the
side-information-privacy problem of Tandon, Sankar, and Poor
\cite{tandon2013} and the privacy funnel of Makhdoumi, Salamatian,
Fawaz, and M\'edard \cite{makhdoumi2014}, which price the leakage
$I(\hX;S)$ of the description about the side information. Under the
Markov chain $I(X;\hX\mid S)=I(X;\hX)-I(\hX;S)$, so the present problem
rewards the leakage that the privacy line penalizes: the two share
their coordinates with opposite signs.
...
\cite{esposito2011} and with side information
\cite{sagawa2012,parrondo2015}; the single-shot sharpenings
\cite{delrio2011,faist2015} price the same reset by smooth entropies,
and Remark~\ref{rem:thermo} states the reading, its setting, and its
scope.

The relation to the information bottleneck \cite{tishby1999} is exact.
For any channel with $\hY-(Y,V)-S$, writing $T=(Y,V)$, the weighted
objective of Section~\ref{sec:region} collapses by the Markov chain:
\begin{equation}
...
structure of the unconstrained Lagrangian, and Meidlinger, Winkelbauer,
and Matz \cite{meidlinger2014} connected it to mean-square
rate--distortion quantization on the rate coordinate; the hard
$Y$-distortion constraint binds a reconstruction error rather than a
mutual information, and the two-noise-fraction frontier is what it adds.

Finally, \cite{bond2026landauer} is a preliminary manuscript by the
author, archived at doi:10.5281/zenodo.21776291, where the discrete
operational region theorem, its single-letterization, and the scalar
Gaussian corner first appeared; everything from Section~\ref{sec:pair}
onward is developed here, with the pair-sufficiency reduction credited
to Armstrong's source-side sufficiency \cite{armstrong2026} as a
distortion-preserving specialization.
...
parties, shown in Fig.~\ref{fig:system}. Let $(X,S)\sim p_{XS}$, and let
$(X^n,S^n)$ be i.i.d. copies. The theorem of this section is for a
general discrete source letter $X$; the Gaussian specialization, in which
$X$ becomes the pair $T=(Y,V)$ and $S=V+U$, is fixed once, at the start
of Section~\ref{sec:pair}. The encoder observes $X^n$
...
Section~\ref{sec:intro} retains $S^n$ and holds the stored index $M$; it
never decodes.
...
\end{equation}
We write $M=f_n(X^n)$ and $\hXn=g_n(M)$. Randomized encoders do not enlarge
the region below: the converse allows them and the achievability does not
need them.
...
\end{align}
The dimensionless quantity $L_n$ is the uncertainty about the stored
index that remains given the retained side information, the equivocation
of the index at the third party in the sense of the secrecy literature
(Section~\ref{sec:intro}). We call it the \emph{conditional content} of the
stored description, because here it is a cost to be made small rather
than a protection to be made large. Every statement and proof of the
paper treats $L_n$ as the coding quantity \eqref{eq:work-def}, in bits per
source symbol.

\begin{definition}\label{def:achievability}
...
operational meaning. The following definition fixes the two notational
scopes of the paper.
...
\begin{remark}\label{rem:convgeneral}
Lemma~\ref{lem:convexity} holds for standard Borel alphabets, with the
sums over $s$ replaced by integrals against the law of $S$, the
conditional laws furnished by regular conditional probabilities, and
conditional mutual information defined as the expected divergence of the
conditional laws \cite{wyner1978}: the proof uses only the joint
convexity of relative entropy \cite{csiszar1967} and the linearity of
the induced output law in the channel, and the convexity holds with
values in $[0,\infty]$. Theorem~\ref{thm:function}(b) and Theorem~\ref{thm:gaussop}
use this form.
\end{remark}
...
\paragraph*{Outline of the coding theorem}
The proof of Theorem~\ref{thm:main-region} below has six steps, and one
point comes first: the reconstruction
...
\begin{theorem}\label{thm:main-region}
...
consequently no time sharing is required, and every Pareto-optimal
point of the region is attained by a single test channel.
\end{theorem}
...
entropy (the standard single-letterization, as in \cite{wynerziv1976}). Likewise,
\begin{align}
 H(M\mid S^n)
 &\ge I(X^n;M\mid S^n)
...
limit point of $(R_n,L_n)$, a bounded sequence by
Definition~\ref{def:achievability}, and with it $(R,L)$, in the right side of
...
encoder selects the smallest index among the codewords jointly
(strongly) typical with $x^n$, or index $1$ if there is none, and stores
that index $M$; the decoder
reconstructs the codeword from $M$ alone. There are two failure events. First,
...
$\Pr(E_{\mathrm{bin}})\to0$ averaged over $(X^n,S^n,\mathcal C,b)$: it
outputs the unique codeword in bin $B$ jointly typical with $S^n$ and
declares an error otherwise. If $I(S;\hX)=0$, so that
$I(X;\hX\mid S)=I(X;\hX)$, the number of bins exceeds the number of
codewords and the bin index determines $M$ with high probability; the
conclusion is unchanged. This decoder is a proof device; the actual
decoder of the code still receives no side information.
...
The achievability construction delivers finite-blocklength guarantees
that Appendix~\ref{app:gaussian} invokes on a quantized surrogate; we
record them as a lemma.

\begin{lemma}\label{lem:construction}
Let $(X,S)$ be a discrete memoryless source with finite alphabets, $d$ a
distortion with maximum $d_{\max}$, $p(\hat x\mid x)$ a test channel
with $\hX-X-S$ and finite reproduction alphabet, and $\epsilon>0$. There
is a sequence $\epsilon_n\to0$, depending on the joint law, the channel,
$\epsilon$, and, through the typicality tolerance that keeps the covered
distortion within $\epsilon$, on $d_{\max}$ and the alphabet sizes, such
that for all sufficiently large $n$ there is a deterministic code
with $|\mathcal M_n|\le2^{n(I(X;\hX)+2\epsilon)}$, distortion
$D_n\le\E\,d(X,\hX)+\epsilon+2\epsilon_nd_{\max}$, and
\[
H(M\mid S^n)\;\le\;n\bigl(I(X;\hX\mid S)+3\epsilon\bigr)
+1+2\epsilon_n\,n\bigl(I(X;\hX)+2\epsilon\bigr).
\]
\end{lemma}

\begin{proof}
This is the selected code of the achievability part of the proof of
Theorem~\ref{thm:main-region}, with the constants displayed at
\eqref{eq:entropy-ach}; $\epsilon_n$ is the averaged failure
...
that residual uncertainty remains in the stored index.
...
thermodynamics with side information \cite{sagawa2012,parrondo2015}: the
third party, acting as the reset mechanism, holds the record $M$ and the
classical side information $S^n$, which is read without disturbance and
remains unchanged, and it holds no other record correlated with $M$;
neither the source $X^n$ nor the reproduction $\hY^n$ is available to it
or coupled to $M$ during the reset (with $X^n$ available, $M=f_n(X^n)$
could be uncomputed at no work cost \cite{bennett1973}). It knows the
joint law of $(M,S^n)$ induced by the code; it may drive $M$ by any
protocol in contact with a heat bath at temperature $T_{\mathrm b}$,
whose driving may depend on $S^n$ but not on $M$; the work supplied may
fluctuate from run to run and only its mean is counted; and only $M$ is
required to return to a standard state. The memory is the ideal logical
memory of Landauer's setting: its logical states are symmetric, with no
free-energy difference between them (equal energies and equal internal
entropies, the condition of \cite{sagawa2009}), the protocol is cyclic in
the memory's Hamiltonian, and the side-information subsystem is left
unchanged; the identity below is the infimum in that model, not a claim
about a particular device. Under these conditions the generalized second
law with correlations \cite{sagawa2012,parrondo2015} bounds the average
work of any such process from below by $\kb T_{\mathrm b}\ln2\,H(M\mid
S^n)$: Landauer's bound \cite{landauer1961}, valid for arbitrary
distributions \cite{esposito2011}, with the entropy of the record reduced
by its mutual information with the retained side information. The bound
is approached, though not reached in finite time, by running for each
value $s$ of $S^n$ the optimal erasure of the law $p(m\mid s)$: a quench
of the level energies to $-\kb T_{\mathrm b}\ln p(m\mid s)$, a
quasi-static isothermal compression to the standard state, and a quench
back to degeneracy \cite{esposito2011}; the three work contributions sum
to $\kb T_{\mathrm b}\ln2\,H(M\mid S^n=s)$, and averaging over $s$ gives
the infimum of the average work over protocols,
\begin{equation}
 W_n^{\star}\;=\;\kb T_{\mathrm b}\ln2\,H(M\mid S^n)\;=\;n\,\kb T_{\mathrm b}\ln2\,L_n ,
 \label{eq:landauer-bound}
\end{equation}
for every code. A mechanism that reads $S^n$ at a finite resolution
$S_\Delta^n$ pays $\kb T_{\mathrm b}\ln2\,H(M\mid S_\Delta^n)\ge\kb
T_{\mathrm b}\ln2\,H(M\mid S^n)$, and the excess vanishes as the
resolution is refined, by the monotone convergence of mutual information
under refining partitions used in Appendix~\ref{app:gaussian}; the
infimum is over resolution as well as over speed. Hence, by
Theorem~\ref{thm:main-region}, and by Theorem~\ref{thm:gaussop} for the
Gaussian source, $L(D)$ is the infimum of the asymptotic average erasure
work, in units of $\kb T_{\mathrm b}\ln2$ per source symbol, over codes
meeting the distortion constraint. This is the average-work setting. The
single-shot, deterministic-work setting of \cite{delrio2011,faist2015}
prices the same reset by smooth conditional entropies of the pair
$(M,S^n)$, and we make no claim in that setting. The scope of the claim:
\eqref{eq:landauer-bound} prices only the logically irreversible reset
of the volatile index $M$; it does not count the work of acquiring $X^n$
or driving a channel \cite{bennett1973}, of correcting errors
\cite{landauer1961}, or of finishing in finite time, and the codebook,
wiring, and any permanent lookup tables are part of the apparatus and are
not erased in each cycle. Physical units are restored by multiplying any
content value in bits per symbol by $\kb T_{\mathrm b}\ln2$.
\end{remark}
...
that must not corrupt reproducibility, and $I(X;\hX\mid S)$ is the
transmitted rate, discounted by the decoder's side information; here $S$
is held by a third party that never serves the decoder, the transmitted
rate is the undiscounted $I(X;\hX)$, and $I(X;\hX\mid S)$ is a second
cost, paid at the third party, that the common-reconstruction problem
never constrains. The two coordinates are therefore constrained
separately, and the object of the paper is their joint region rather
than either endpoint. The remainder of the paper characterizes $L(D)$,
and the region around it, for the source of Section~\ref{sec:pair}.
...
is proved there in full, and the achievability by quantizing the side information, the source, and the
...
Symbol & Meaning\\
\hline
$T=(Y,V)$ & the encoder-observed pair, described variable $Y$ and
context $V$, normalized in Section~\ref{sec:pair}. $X$ is the source letter of
the discrete theorem (Section~\ref{sec:operational}) and, in the Gaussian
sections, the ambient vector from which $T$ is drawn\\
$S=V+U$ & the side information, a noisy copy of the context; $U\sim\mathcal N(0,\tau^2)$
independent of $X$\\
$s=1+\tau^2$ & the context-noise parameter, the variance of $S$ after
normalization (scalar side information;
the vector-context section uses $\Sigma_U$, with side-information
covariance $\Sigma_S=\Sigma_V+\Sigma_U$)\\
...
$g^\star$; $g_{\mathrm m}$, $g_f$, $g_R$ & the root giving
$L(D)=\tfrac12\log_2 g^\star$ (Theorem~\ref{thm:function}); the roots
compared with it: Steinberg's scalar formula on $(Y,S)$
(Corollary~\ref{cor:margstrict}), the conditional-variance bound
(Remark~\ref{rem:floor}), and Lu and Xu's largest admissible value
(Appendix~\ref{app:lucompare})\\
\hline
\end{tabular}
\end{table}
...
force and is stated once. The source letter is the pair $T=(Y,V)$, drawn
from the ambient vector $X$ above, which enters only through $T$ once
Lemma~\ref{thm:pair} is proved; the distortion is $(y-\hat y)^2$ on the
$Y$ coordinate, so the decoder is judged on $Y$ alone while the encoder
observes the pair; the reproduction is $\hY$; and all information
quantities are written on $(T,\hY,S)$. Reproductions may be
...
The Gaussian sections run under $0<D<1$, $|\rho|<1$, and $\tau^2>0$;
each boundary is treated where it arises.
For $D\ge1$ the zero reproduction gives $R=L=0$, and at $D=0$ no finite
pair is achievable; both are recorded with the Gaussian operational
theorem (Appendix~\ref{app:gaussian}). The boundaries $\rho^2\in\{0,1\}$
are two of the three corners of Corollary~\ref{cor:anchors}, the
classical function at $\rho=0$ and Steinberg's scalar formula as
$\rho^2\to1$, and the frontier collapse at both is computed in
Corollary~\ref{cor:misalign}. The clean-context boundary
$\tau^2=0$ has its own branch wherever it matters:
Proposition~\ref{prop:marg} and the $\tau^2\to0$ limit for the
function, and Proposition~\ref{rem:cleanboundary} for the frontier; positivity
...
unchanged at every blocklength; Lemma~\ref{thm:pair} is the case in
...
Lemma~\ref{thm:pair} we restrict to $T$-channels, $T=(Y,V)$, with
scalar reproduction $\hY$. We first fix the working coordinates. Every
jointly Gaussian test channel
...
The converse rests on one
estimation-theoretic lemma. We state it for vector reproductions and
vector side information, in the generality that
...
Conversely, \eqref{eq:lem-cond} holds with equality only if the
conditional law of $T$ given $(\hat{\mathbf Y},S)$ is almost surely
Gaussian with mean the best linear estimate and covariance $\Sigma_e$;
likewise for \eqref{eq:lem-rate} with $S$ deleted.
\end{lemma}

\begin{proof}


\emph{Step 1: the divergence bound.} On standard Borel spaces,
conditional mutual information is the expected divergence of the
conditional law of $T$ given $(\hat{\mathbf Y},S)$ from the conditional
law given $S$ \cite{wyner1978,pinsker1964}:
$I(T;\hat{\mathbf Y}\mid S)=\E\,D\bigl(P_{T\mid\hat{\mathbf Y},S}\,\big\|\,
P_{T\mid S}\bigr)$, with the expectation over $(\hat{\mathbf Y},S)$ and
the value $+\infty$ allowed. Fix $(\hat y,s)$ outside a null set, write
$P=P_{T\mid\hat{\mathbf Y}=\hat y,S=s}$, let $\mu$ and $C$ be its mean
and covariance, let $P_G=\mathcal N(\mu,C)$, and let
$Q=P_{T\mid S=s}=\mathcal N(\mu_S(s),\Sigma_{T\mid S})$. If $P$ has no
Lebesgue density, $D(P\|Q)=\infty$. Otherwise, because
$\log(dP_G/dQ)$ is a quadratic polynomial in $t$ and $P$ and $P_G$ share
their first and second moments, $\E_P\log(dP_G/dQ)=\E_{P_G}\log(dP_G/dQ)$,
so $D(P\|Q)=D(P\|P_G)+D(P_G\|Q)\ge D(P_G\|Q)$, with equality if and only
if $P=P_G$. In nats,
$D(P_G\|Q)=\tfrac12\bigl[\tr\bigl(\Sigma_{T\mid S}^{-1}(C+\delta\delta^{\mathsf T})\bigr)
-p+\ln\det\Sigma_{T\mid S}-\ln\det C\bigr]$ with $\delta=\mu-\mu_S(s)$,
and $-\ln\det C=+\infty$ if $C$ is singular. Take expectations. By the
law of total covariance, $\E[C+\delta\delta^{\mathsf T}\mid S]=\Sigma_{T\mid S}$,
so the trace term averages to $p$. By concavity of $\ln\det$ and
Jensen's inequality, $\E[-\ln\det C]\ge-\ln\det\E[C]$, and
$\E[C]=\Cov\bigl(T-\E[T\mid\hat{\mathbf Y},S]\bigr)\preceq\Sigma_e$,
because the conditional mean has error covariance no larger than that of
any linear estimator. Hence
$I(T;\hat{\mathbf Y}\mid S)\ge\tfrac12\log_2\bigl(\det\Sigma_{T\mid S}/\det\Sigma_e\bigr)$,
the inequality in \eqref{eq:lem-cond} once Step 2 evaluates
$\det\Sigma_e$. Equality holds only if $P=P_G$ almost surely, $C=\Sigma_e$
almost surely (the equality case of Jensen's inequality, $\ln\det$ being
strictly concave), and $\E[T\mid\hat{\mathbf Y},S]$ is the best linear
estimate almost surely, since $\Cov(T-\widehat T)=\E[C]+
\Cov(\E[T\mid\hat{\mathbf Y},S]-\widehat T)$; the conditional law of $T$
given $(\hat{\mathbf Y},S)$ is then $\mathcal N(\widehat T,\Sigma_e)$, so
$T-\widehat T$ is Gaussian and independent of $(\hat{\mathbf Y},S)$. The
$S$-deleted case is identical with $Q=\mathcal N(0,\Sigma_T)$.

\emph{Step 2: the determinant identity.} Write
$\Omega:=\Cov\bigl((\hat{\mathbf Y},S)\bigr)$ and
$J:=\Cov\bigl(T,(\hat{\mathbf Y},S)\bigr)\in\R^{p\times(m+r)}$. First,
$\Omega\succ0$: by the Schur-complement criterion on
\[
\Omega=\begin{bmatrix}
...
$\Cov(T,S)\succ0$, and the second is evaluated below to equal
$A\Sigma_{T\mid S}A^{\mathsf T}+\Sigma_N\succeq\Sigma_N\succ0$. The best
linear estimator of $T$ from $(\hat{\mathbf Y},S)$ is
$\widehat T=J\Omega^{-1}(\hat{\mathbf Y};S)$, with error covariance
$\Sigma_e=\Sigma_T-J\Omega^{-1}J^{\mathsf T}$. Let
...
$\det K=\det\Omega\,\det\Sigma_e$, and applied to $\Omega$ itself gives
\[
\det\Omega=\det\Sigma_S\cdot\det\Bigl(\Cov(\hat{\mathbf Y})
...
\[
\det\Sigma_e=\frac{\det K}{\det\Omega}
...
$r=0$: $K=\Cov(T,\hat{\mathbf Y})$, $\Omega=\Cov(\hat{\mathbf Y})$) gives
...
law of $T$ given $(\hat{\mathbf Y},S)$ is $\mathcal N(\widehat T,\Sigma_e)$
for every realization, so every inequality of Step 1 is an equality and
\eqref{eq:lem-cond} holds with equality; likewise for \eqref{eq:lem-rate}. The channel's moments match those of
...
zero and may be dropped, reducing $m$; iterate until $\Sigma_N\succ0$. All uses of
Lemma~\ref{lem:gauss} below may therefore assume $\Sigma_N\succ0$.
\end{remark}

\begin{theorem}\label{thm:function}\label{thm:region}
With $s=1+\tau^2$, the context-noise parameter of
Section~\ref{sec:pair}, for $0<D<1$:

\emph{(a) The closed form.}
\begin{equation}
...
\emph{(b) The region.} The union of the quadrants above the single-letter
rate--content pairs
...
(Proposition~\ref{prop:uniq} in Section~\ref{sec:region}; at $\alpha=0$
uniqueness also follows from part (a)), and the frontier point is
\[
...
and value of part (a) ($\gamma_1=1/g^\star$).
\end{theorem}

Part (a) is proved next. Part (b) is proved in Section~\ref{sec:region},
after Proposition~\ref{prop:uniq} there supplies the uniqueness it uses.

\begin{proof}[Proof of part (a)]
\emph{Moments.} By Lemma~\ref{thm:pair} and the scalar-reproduction
...
\begin{corollary}\label{cor:anchors}
...
\cite{gray1973}, matching Proposition~\ref{prop:marg}; for $\tau^2>0$,
$\rho\ne0$, and $D<1-\rho^2$ the value lies strictly above Gray's.
(iii) As $\rho^2\to1$,
...
for $D\ge1-\rho^2$ it is $0$. Strictness for $\tau^2>0$: at
$g_G=(1-\rho^2)/D>1$,
$P(g_G)=-\rho^2\tau^2(1-\rho^2)/D<0$ when $\rho\ne0$, so $g_G$ lies
between the two roots and $g^\star>g_G$.
...
\begin{corollary}\label{cor:margstrict}
Under the standing model, for every $\rho\ne0$ and $0<D<1$,
$L(D)<\tfrac12\log_2 g_{\mathrm m}$ with $g_{\mathrm m}=(s-\rho^2+\rho^2D)/(sD)$, the value
of Steinberg's scalar formula \cite{steinberg2009} on the reduced pair
$(Y,S)$; so a channel that depends on $X$ through $Y$ alone is strictly
suboptimal.
\end{corollary}

\begin{proof}
The marginalized class attains exactly $\tfrac12\log_2g_{\mathrm m}$, the
scalar-corner value of the reduction preceding
Proposition~\ref{prop:marg} at $\rho_{YS}^2=\rho^2/s$. Substituting into
\eqref{eq:Pg} gives $P(g_{\mathrm m})=\rho^2(1-D)(1-\rho^2)/s>0$ and
$g_{\mathrm m}-1=(1-D)(s-\rho^2)/(Ds)>0$. Since $P(1)=(D-1)\tau^2<0$, the two
roots of $P$ bracket $1$; a point above $1$ at which $P>0$ lies above
the larger root, so $g_{\mathrm m}>g^\star$. The same value
$\tfrac12\log_2 g_{\mathrm m}$ reappears in Corollary~\ref{cor:misalign} as
the conditional content of the rate-optimal channel, which lies in the
marginalized class.
\end{proof}
...
Attaining the content endpoint of Theorem~\ref{thm:function} costs rate.
This section characterizes the full tradeoff between the
two coordinates of \eqref{eq:main-region} for the Gaussian pair source.
Throughout, $|\rho|<1$, $\tau^2>0$, $0<D<1$, and $h$, $Q_0$,
...
are the two noise fractions the stationarity system balances, the
shares of the reproduction's unconditional and $S$-conditional variance
that its noise occupies: the frontier point at weight $\alpha$ is
$\bigl(\tfrac12\log_2(1/\gamma_0),\,\tfrac12\log_2(1/\gamma_1)\bigr)$, and
the system couples the two fractions through one reproduction and one
distortion constraint. The proof of part (b) uses the uniqueness of the
weighted minimizer, so the whitened frame, Lemma~\ref{lem:mxconvex}, and
Proposition~\ref{prop:uniq} come first.


Uniqueness at every weight is a convexity statement in moment
coordinates, stated in the whitened frame that Section~\ref{sec:vector}
also uses; we set it up once. Since
...
\begin{lemma}\label{lem:mxconvex}
...
\begin{proof}
For $\sigma>0$ the Schur-complement criterion \cite[Sec.~A.5.5]{boyd2004} states
\[
...
its global minimizers and the minimizer set is convex; coercivity gives
attainment, since on the slice $c$ ranges over the closed ball
$|y_0-c|^2\le D$, which excludes $c=0$ because $D<1$, so
$Q_1\ge\lambda_{\min}|c|^2$ is bounded away from zero and
$B_\alpha\to\infty$ as $\nu\to0$.
...
alone, whose convexity (Lemma~\ref{lem:mxconvex} with $u(z)=c$ and
...
holds for arbitrary side information, including the boundary $\tau^2=0$,
where $\Sigma_{T\mid S}$ is singular and the whitening
\eqref{eq:whitening} of the conditional covariance is unavailable.
\end{proof}

We now prove part (b) of Theorem~\ref{thm:function}, the region and its
frontier.

\begin{proof}[Proof of part (b)]
\emph{Convexity and scalarization.} Both coordinates are convex in the
test channel, by Lemma~\ref{lem:convexity} in the general-alphabet form
of Remark~\ref{rem:convgeneral}.
The achievable set of pairs at distortion $D$ is therefore convex and
upward closed in each coordinate (and closed, by the closedness step of
Appendix~\ref{app:gaussian}), so every point of its Pareto frontier
minimizes one of the weighted objectives
\[
J_\alpha:=\alpha\,I(T;\hY)+(1-\alpha)\,I(T;\hY\mid S),
\qquad\alpha\in[0,1],
\]
...
(Proposition~\ref{prop:uniq}; the argument is run in detail in the
proof of Corollary~\ref{cor:misalign}).

\emph{The two determinant identities.} Fix an admissible channel,
centered without loss, with regression coordinates $(a,b,\nu)$, $\nu>0$,
as in the proof of part (a); the Markov moment
identity $\Cov(N',S)=0$ holds as there. Lemma~\ref{lem:gauss} applies
twice to the same three moments, once with the side information $S$ and
once with $S$ deleted:
\begin{equation}
I(T;\hY)\ \ge\
...
out in the proof of part (a). The rate identity, in
...
\end{equation}
As in the proof of part (a): $\varphi_\alpha$ is strictly
...
b=(1-\alpha)\gamma_1\mu_c ,
\]
dividing by $1-\rho^2>0$, which is in force since $|\rho|<1$
...
point of the reduced program, and by Proposition~\ref{prop:uniq}
...
become \eqref{eq:foc} with $g=1/\gamma_1$; part (a)
...
\]
the content of the classical reverse channel, which equals
$\tfrac12\log_2 g_{\mathrm m}$, Steinberg's scalar formula on the reduced
pair $(Y,S)$ of Corollary~\ref{cor:margstrict}: the rate-optimal channel
ignores the context, and the content it pays is the marginalized-class
optimum; and
$\RLs(D)=\tfrac12\log_2\bigl[(Q_0+\nu)/\nu\bigr]$ evaluated at the channel of
Theorem~\ref{thm:function} strictly exceeds $R_{\min}$. When $\rho=0$ the classical reverse channel
...
(Proposition~\ref{rem:cleanboundary}).
\end{corollary}

\begin{proof}
By part (a) of Theorem~\ref{thm:function}, the moment program for the conditional
...
the proof of part (b) (uniqueness by
...
Proposition~\ref{rem:cleanboundary} evaluated at $D=1-\rho^2$. All quoted
values are computed by the scripts of Appendix~\ref{app:numver}. Fig.~\ref{fig:frontiermulti} repeats the construction at
$\rho^2\in\{0.2,0.5,0.8\}$ with $\tau^2=0.5$ and $D=0.3$; the frontier is a
nondegenerate curve at each, as Corollary~\ref{cor:misalign} states.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{frontier}
\caption{The rate--content region $\RW(D)$ (shaded) and its Pareto
frontier at $(\rho^2,\tau^2,D)=(0.75,0.5,0.3)$, the second instance of
Table~\ref{tab:notmarginal} at $D=0.3$, traced by the two-noise-fraction
system of Theorem~\ref{thm:region}(b). The tradeoff is strict: the rate-optimal channel
...
$\rho^2=0$, is widest at intermediate correlation, and closes again toward
the $\rho^2\to1$ boundary. Marker shape distinguishes the curves.}
\label{fig:frontiermulti}
\end{figure}

\begin{proposition}\label{rem:cleanboundary}
...
side information, and at $\alpha=1$ the uniqueness proof of
...
forces its moments to be that triple and forces equality in
\eqref{eq:lem-rate}, whose equality clause (Lemma~\ref{lem:gauss}) makes
the conditional law of $T$ given $\hY$ Gaussian with mean $J\hY$,
$J=\E[T\hY]/\operatorname{Var}\hY$, and covariance $\Sigma_{e0}$: the
linear-estimation error $E:=T-J\hY$ is Gaussian and independent of
...
$(Y,S)$ nor Gray's, can equal $L$. Steinberg's functional is unchanged;
its source here is the full pair $(Y,V)$, and the augmentation is
essential: the encoder's access to the clean context $V$ enters the
function.
\end{corollary}
...
$g^\star=(17+\sqrt{229})/6$ at $D=0.1$ and $(7+\sqrt{29})/6$ at
...
which the $\tau^2\to0$ limit of Corollary~\ref{cor:anchors} matches). The instance gives
\[
...
\qquad
g^\star=\frac{7+\sqrt{29}}{6}=2.0642,
\qquad
...
The reason is as follows. Both instances have the same
...
$0.1543$ bits at $D=0.3$. A pair inside the standing model shows the
same: $(\rho^2,\tau^2)=(\tfrac35,\tfrac15)$ and $(\tfrac34,\tfrac12)$
share $\rho_{YS}^2=\tfrac12$, and for the first the closed form gives
$g^\star=(35+\sqrt{745})/12$ and $(15+\sqrt{65})/12$, so $L=1.1880$ and
$0.4712$ bits at $D=0.1$ and $0.3$, against the second instance's
$1.2105$ and $0.5228$.

\begin{remark}\label{rem:floor}
The closed form settles when the conditional-variance lower bound is
attained. Every admissible channel obeys
$L\ge I(Y;\hY\mid S)=h(Y\mid S)-h(Y\mid\hY,S)\ge h(Y\mid S)-h(Y-\hY)
\ge\tfrac12\log_2\bigl(\operatorname{Var}(Y\mid S)/D\bigr)
=\tfrac12\log_2 g_f$ with $g_f=(s-\rho^2)/(Ds)$, by the Gaussian
maximum-entropy bound at variance $D$. Within the standing model
$\tau^2>0$ the bound, taken as its positive part where $g_f<1$, is
attained exactly when $\rho=0$: substituting $g_f$ into \eqref{eq:Pg}
gives $P(g_f)=-\rho^2\tau^2/s$, negative when $\rho\ne0$, so that
$g^\star>g_f$ strictly; at $\rho=0$ the quadratic factors as
$(Dg-1)(sg-1)$ and $g_f=1/D$ is its larger root. The clean-context
case $\tau^2=0$, where the bound is Gray's function and is attained on
$D\le1-\rho^2$, is Proposition~\ref{rem:cleanboundary}.
\end{remark}

\section{Vector Context}
\label{sec:vector}

The frontier survives vector side information. Let
...
the unique solution with $\nu>0$ of the generalized two-noise-fraction
system
\begin{equation}
...
For $r=1$ the program coincides with that of Theorem~\ref{thm:region}(b)
...
\begin{proof}
(i) In the resampling proof of Lemma~\ref{thm:pair}, the only
...
where $\Sigma_S=\Sigma_V+\Sigma_U$ is the side-information covariance
...
Theorem~\ref{thm:region}(b): both coordinates are convex in the channel,
...
\emph{The first-order system.} Differentiating
$\alpha\ln(Q_0+\nu)+(1-\alpha)\ln(Q_1+\nu)-\ln\nu$ in $c$ with
$\nu=D-|y_0-c|^2$, exactly as in the proof of
Theorem~\ref{thm:function}(b) with $\nabla Q_0=2c$ and
$\nabla Q_1=2\Lambda c$, and multiplying by $\nu/2$ gives
\eqref{eq:vecfoc}; the bracket is invertible, each diagonal entry being
a convex combination of $1$ and $\lambda_j>0$. Uniqueness of the
minimizer, hence of the system's solution with $\nu>0$, at every
$\alpha\in[0,1]$ is Proposition~\ref{prop:uniq} at $p=1+r$, whose proof
is dimension-free; achievability of the frontier point is (ii).
...
substituting into $\nu=D-|y_0-c|^2$ and
$\gamma_1=\nu/(c^{\mathsf T}\Lambda c+\nu)$ leaves one scalar equation in
$\gamma_1\in(0,1)$, the vector-context generalization of the
quadratic \eqref{eq:Pg}.
\end{proof}

\section{The Binary Frontier}
\label{sec:binary}

The binary case serves two purposes: it shows that the
...
(Proposition~\ref{rem:binfrontier}), and it shows the functional is
explicitly computable outside the Gaussian family. The conditional
content itself coincides with a known common-reconstruction rate
(Appendix~\ref{app:lucompare}); the frontier is traced here, unlike its
Gaussian counterpart, by an elementary self-contained argument.
...
(a richer description cannot help: for any stored $Z$ from which the
...
\begin{lemma}\label{lem:binconvex}
...
\begin{proof}
Work in nats, with $h_e$ the binary entropy function in nats. On the
symmetric family the value is
$L(d_0)=h_e(u)-(1-p)h_e(d_0)-p\,h_e\bigl(d_1(d_0)\bigr)$, the closed form
computed in the proof of Theorem~\ref{thm:binary} from the family's
symmetry alone; the $L$ of that theorem in bits is this $L$ divided by
$\ln2$. Differentiating
...
with values in $(0,1)$, $u=a(1-2q)+q\in(0,1)$, and $h_e$ is real analytic
...
rate--content frontier at representative instances. At
$(p,q,D)=(0.25,0.15,0.15)$ the residual crosses zero once, at
$d_0^\star=0.1173$ ($d_1^\star=0.2480$), where $L$ attains its minimum
$0.3346$ bits. At $(0.1,0.1,0.05)$ the frontier runs from the tilt root
$d_0^\star=0.0282$, where $L_{\min}=0.4341$ bits, to $d_0=D$, where
$R_{\min}=1-h_2(D)=0.7136$ bits; the $L$-optimal point pays $0.0391$ bits
of excess rate and the $R$-optimal point $0.0248$ bits of excess
conditional content.
...
$\psi(d_0)$ (middle) along the constraint segment at
$(p,q,D)=(0.25,0.15,0.15)$, and the binary rate--content frontier
(bottom) at $(p,q,D)=(0.1,0.1,0.05)$ with the Pareto arc from the tilt
root to the rate minimizer drawn bold. The residual crosses zero once,
and the arc is a strict tradeoff: each endpoint pays a positive excess in
the other coordinate.}\label{fig:binary}
\end{figure}
...
constraint $(1-p)d_0+pd_1=D$ together with the \emph{tilt equation}, a
balance of log-odds,
\[
...
$\hY=V$.
(In the excluded case $q=0$, $D>p$, the constraint is inactive and
...
$u=\Pr[\hY\ne S]$). Special cases: at $q=0$ the system gives
...
\emph{Special cases and positivity.} $q=\tfrac12$: $u=\tfrac12$, the tilt
...
\begin{proposition}\label{rem:binfrontier}
The symmetric family exhausts not only the conditional coordinate but
the entire rate--content frontier at distortion $D$: the Pareto
frontier of the binary rate--content region is the arc $d_0\in[d_0^\star,D]$
of the curve
\[
...
\]
along the constraint segment. On the segment $R$ is uniquely minimized
at $d_0=d_1=D$, where $R=1-h_2(D)$ is the marginal rate--distortion
function, and $L$ at the tilt root $d_0^\star$ of
Lemma~\ref{lem:tiltroot}. For $q\in(0,\tfrac12)$ the two minimizers are
distinct, $d_0^\star<D$, and $[d_0^\star,D]$ is a strict tradeoff arc,
$L$ increasing and $R$ decreasing, with both coordinates worsening
outside it; at $q=\tfrac12$ the frontier is a single corner.
\end{proposition}

\begin{proof}
The rate
coordinate $R=I(Y,V;\hY)$ is convex in the shared channel by the
argument of Lemma~\ref{lem:binconvex} with $S$ deleted, so
...
continuously to $\tfrac12>D$ and meets the budget on the way. Hence the
frontier lies on the displayed curve along the segment, and the arc
between the two minimizers identified next is the Pareto-optimal part. On the segment, $R$ is strictly convex
...
The conditional-content function of this section is the
common-reconstruction rate of Lu \emph{et al.} \cite{lu2016lossy} under
the relabeling of Appendix~\ref{app:lucompare}; the contribution here is
the joint $(R,L)$ frontier. Two further common-reconstruction
calculations are decoder-side-information ones.
...
$\Pr[\hY\ne Y]$. The function meets Gray's conditional
...
alphabet-free); the tilt equation is what the encoder's access to the
pair adds.
...
whose encoder observes the pair $(Y,V)$ while the side information $S$
stays with a third party: the closed form of Theorem~\ref{thm:function}(a) for $Y$ and
$V$ scalar linear functionals of an arbitrary-dimensional jointly Gaussian
source, the exact rate--content region of Theorem~\ref{thm:region}(b) with
its two-noise-fraction frontier and the misalignment condition of
Corollary~\ref{cor:misalign}, the attainment condition for the conditional-variance bound
(Remark~\ref{rem:floor}), the vector-context extension of
Theorem~\ref{thm:vector}, and the binary rate--content frontier of
Theorem~\ref{thm:binary} and Proposition~\ref{rem:binfrontier}. The function degenerates to the classical
...
description content, the rate at which a party holding the side
information can re-encode the description, is a property of the encoding
configuration, not of the source and side-information pair alone.
...
The closed forms were checked numerically against direct minimization
...
Gaussian source of Section~\ref{sec:pair} without appeal to an
abstract-alphabet extension of the finite-alphabet theorem. The source letter
is the pair $T=(Y,V)$ of Section~\ref{sec:pair}, jointly Gaussian, as
Definition~\ref{def:problem} fixes it; a code whose encoder observes the
ambient source of Section~\ref{sec:pair} is covered as well, since any
$T^n$-code is such a code, and for any ambient channel the resampled
$T$-channel of Lemma~\ref{thm:pair} keeps the distortion and the
conditional coordinate and has rate $I(T;\hY')\le I(X;\hY)$, so the union
over ambient channels equals the union over $T$-channels. The
side information is $S\in\R^r$ as in
...
achievability is Definition~\ref{def:achievability}. The proof
establishes, in order: the converse by the chain rule; restriction to
Gaussian channels; closedness and right-continuity of the union; and
achievability in three steps, quantizing the side information
(Step A), then the source and the reproduction (Step B, with a finite
reproduction alphabet at every stage and the Markov chain preserved by
construction), then returning to the unquantized side information through
$H(M\mid S^n)\le H(M\mid S_\Delta^n)$ (Step C). The rate and content
coordinates converge by data processing, martingale convergence, and
lower semicontinuity of mutual information; the distortion is
transferred exactly by memorylessness rather than by weak convergence.

One device is fixed first, the \emph{saturating dyadic quantizers}:
for $\kappa\ge1$, $q_\kappa$ partitions $\R$ into the bounded cells
$[j2^{-\kappa},(j+1)2^{-\kappa})$, $-4^\kappa\le j<4^\kappa$, and the two saturating tail
cells $(-\infty,-2^\kappa)$ and $[2^\kappa,\infty)$, and maps each point of a
bounded cell to the cell's left endpoint and the two tail cells to
$-2^\kappa$ and $2^\kappa$, so that $|q_\kappa(u)-u|\le2^{-\kappa}+|u|\,\mathbf 1\{|u|>2^\kappa\}$
for every real $u$. A vector is quantized coordinatewise, so its cells
are rectangles. The partitions are nested, every cell edge of $q_\kappa$
being an edge of $q_{\kappa+1}$, so for any random vector $Z$ the
$\sigma$-fields $\sigma(q_\kappa(Z))$ increase with $\kappa$, and their union
generates $\sigma(Z)$, since the dyadic rectangles generate the Borel
$\sigma$-field. Write $S_\kappa:=q_\kappa(S)$.

\begin{theorem}\label{thm:gaussop}
For the Gaussian source under the standing assumptions of
Section~\ref{sec:pair} ($\tau^2>0$, $\Sigma_T\succ0$), with the pair
$T=(Y,V)$ as the source letter $X$ (Definition~\ref{def:problem}), and
every $0<D<1$,
\[
...
The compactness step of the proof uses $D<1$. For $D\ge1$ both sides
equal $[0,\infty)^2$: the zero reproduction meets the distortion
constraint with $R=L=0$, and $R_n,L_n\ge0$ for every code. For $D=0$
both sides are empty as subsets of $\R^2$: a code with finitely many
messages has $D_n>0$ and $R_n\ge\tfrac12\log_2(1/D_n)$, so no finite
pair is achievable at distortion $0$, and the only channels with zero
distortion have $\hY=Y$ almost surely, with both coordinates infinite.
Throughout, $L_n$ is the coding quantity
\eqref{eq:work-def}; no step of the proof depends on
Remark~\ref{rem:thermo}.

\begin{proof}
\emph{Converse.} The chain of Theorem~\ref{thm:main-region}'s converse
(the discrete operational region theorem) holds for the continuous
source.
$M$ is discrete, so $\log|\mathcal M_n|\ge H(M)\ge I(X^n;M)\ge
I(X^n;\hY^n)$ and $H(M\mid S^n)\ge I(X^n;M\mid S^n)\ge
I(X^n;\hY^n\mid S^n)$ are unchanged. The single-letterization needs no
differential entropy. Since $X_i$ is independent of $X^{i-1}$, the chain
rule gives
$I(X^n;\hY^n)=\sum_iI(X_i;\hY^n,X^{i-1})\ge\sum_iI(X_i;\hY_i)$; and since
the pair $(X_i,S_i)$ is independent of the other pairs,
$I(X_i;X^{i-1},S^n)=I(X_i;S_i)$, so
$I(X^n;\hY^n\mid S^n)=\sum_i\bigl[I(X_i;\hY^n,X^{i-1},S^n)-I(X_i;S_i)\bigr]
\ge\sum_i\bigl[I(X_i;\hY_i,S_i)-I(X_i;S_i)\bigr]=\sum_iI(X_i;\hY_i\mid S_i)$.
Every term is finite: each is at most $I(X_i;S_i)+H(M)$, where
$I(X_i;S_i)=\tfrac12\log_2(s/\tau^2)$ is finite because $\tau^2>0$ and
$H(M)\le\log|\mathcal M_n|$ because $\hY^n$ is a function of the
finite-valued $M$; this is a second place where $\tau^2>0$ is used. The per-letter induced channels satisfy the
...
Lemma~\ref{lem:convexity} in the general-alphabet form of
Remark~\ref{rem:convgeneral}, which holds for the conditional coordinate
because, under $\hY-X-S$, $I(X;\hY\mid S)$ is the integral over $s$
against $P_S$ of the relative entropy between the joint law
$P_{X\mid S=s}\,q$ of $(X,\hY)$ given $S=s$ and the product
$P_{X\mid S=s}\times(P_{X\mid S=s}q)$ of its marginals; both arguments
are affine in $q$, so the integrand is convex in $q$ by the joint
convexity of relative entropy, and so is the integral. Hence
$(R_n,L_n)$ lies in the displayed union at distortion $D_n$.

\emph{Restriction to Gaussian channels.} For any admissible channel the
hypotheses of Lemma~\ref{lem:gauss} are met after one normalization.
Centering $\hY$ leaves both coordinates and the Markov chain unchanged
and lowers the distortion by $(\E\hY)^2$; $\E\hY^2<\infty$ follows from
$\E(Y-\hY)^2\le D$ and $\E Y^2=1$; the residual of the regression of
$\hY$ on $T$ is uncorrelated with $S$ by the argument in the proof of
Theorem~\ref{thm:function}(a), since $U$ is independent of $(X,\hY)$;
and a positive residual variance may be assumed, because a zero
residual with nonzero regression coefficients gives both coordinates
infinite and an empty quadrant (Remark~\ref{rem:degenerate}), while a
zero residual with zero coefficients is the zero reproduction, infeasible
for $D<1$. With these in place,
...
step, the union at any distortion level $D'\in(0,1)$ equals the
...
$\delta\in(0,1-D)$, so a subsequence converges to some $\theta^*$, feasible at
...
\emph{Achievability, Step A: quantizing the side information.} Fix a jointly
Gaussian admissible channel $p(\hat y\mid x)$ with distortion at most
$D$ and residual variance $\nu>0$ (a channel with $\nu=0$ has both
coordinates infinite by the coordinate formulas of the closedness step,
the conditional one because $\Lambda\succ0$ when $\tau^2>0$, and
contributes nothing to the union), and fix $\epsilon>0$. Along the saturating dyadic quantizers,
\begin{equation}
I(\hY;S_\kappa)\;\uparrow\;I(\hY;S)\qquad(\kappa\to\infty):
\label{eq:quant-mono}
\end{equation}
the sequence is nondecreasing because the partitions are nested (data
processing), and its limit is $I(\hY;S)$ because the $\sigma$-fields
$\sigma(S_\kappa)$ increase to $\sigma(S)$ and mutual information is
continuous along an increasing sequence of $\sigma$-fields whose union
generates the limit \cite[Ch.~2]{pinsker1964}. Choose $\kappa$ with
\begin{equation}
I(\hY;S_\Delta)\;\ge\;I(\hY;S)-\epsilon,
\qquad S_\Delta:=S_\kappa .
...
($I(X;\hY)-I(X;\hY\mid S')=I(\hY;S')$ for any side information $S'$ with
$\hY-X-S'$),
\begin{equation}
...
in $L^2$, since $\E\hY^2<\infty$ by the distortion constraint and
$\E[\hY^2\mathbf 1\{|\hY|>2^m\}]\to0$ by dominated convergence. The
finite-alphabet surrogate consists of the source $X_m$
...
\]
which is bounded, since both alphabets are finite. Both conditional
expectations are well defined, because $\Sigma_T\succ0$ gives every
rectangle of $q_m$ positive probability under the law of $X$.
...
Lemma~\ref{lem:construction} applies to the surrogate and, for every
$\epsilon>0$ and every $m$, supplies a deterministic code, a function of
$X_m^n$ alone, with $|\mathcal M_n|\le2^{n(I(X_m;\hY_m)+2\epsilon)}$,
surrogate distortion at most $\E_md_m+\epsilon+2\epsilon_nd_{\max}(m)$,
and
\begin{equation}
H(M\mid S_\Delta^n)\;\le\;
n\bigl(I_m(X_m;\hY_m\mid S_\Delta)+3\epsilon\bigr)
+1+2\epsilon_n\,n\bigl(I(X_m;\hY_m)+2\epsilon\bigr),
\label{eq:quant-fano}
\end{equation}
where $\E_m$ and $I_m$ are computed under the surrogate joint law,
$d_{\max}(m)=\max_{x_m,\hat y}d_m(x_m,\hat y)$ is finite for each $m$
but grows with $m$ because the tail cells sit at $\pm2^m$, and
$\epsilon_n\to0$ for each fixed $(\epsilon,\kappa,m)$. The limits below are
taken in this order: $\epsilon$ is fixed; $\kappa$ is fixed by Step A; $m$ is
fixed by (ii) and (iii); then $n\to\infty$, so that
$2\epsilon_nd_{\max}(m)=o(1)$ and $2\epsilon_nn(\cdot)=o(n)$, which is
how they are written below; and finally $\epsilon\downarrow0$ along a
diagonal sequence. Also, the
...
which sends the first term to zero, and $\hY_m\to\hY$ in $L^2$. (The
surrogate distortion is not $\E(Y-\hY_m)^2$ under the true law, since
$Y$ and $\hY_m$ are dependent given $X_m$; the argument does not use
that identity.)
...
$\R\times q_\kappa(\R^r)$, continuous in its first argument,
\begin{align*}
\int f\,dQ_m
...
=\E f(\hY,S_\Delta):
\end{align*}
for each $j$, $\E|f(\hY_m,j)-f(\hY,j)|\to0$ by bounded convergence,
since $\hY_m\to\hY$ pointwise and $f$ is bounded and continuous in its
first argument, and $\E[f(\hY,j)\mid X_m]\to\E[f(\hY,j)\mid X]$ in $L^1$
by the martingale convergence theorem, so
$\E[f(\hY_m,j)\mid X_m]\to\E[f(\hY,j)\mid X]$ in $L^1$ by the triangle
inequality; the factors $\Pr(S_\Delta=j\mid X_m)$ are bounded by one and
converge in $L^1$ to $\Pr(S_\Delta=j\mid X)$ by the same theorem, so each
product converges in $L^1$ and the finite sum passes to the limit;
...
\emph{Achievability, Step C: back to the unquantized side information.}
...
$\epsilon_j\downarrow0$ of codes (the $\epsilon_j$-code exists at every
sufficiently large blocklength by Lemma~\ref{lem:construction}, and
blocklengths $N_j\le n<N_{j+1}$ use it) achieves, in the $\limsup$ sense
of Definition~\ref{def:achievability}, the corner
...
The two imported results are the monotone convergence of mutual
...
The closed forms and the displayed matrix identities were checked
independently of the derivations: each closed form numerically, against
direct minimization over test-channel moments, and each identity
symbolically, by computer algebra. The check scripts and an executed
notebook, \texttt{reproduce.ipynb}, which evaluates every closed form
against direct minimization and regenerates the figures and tables,
accompany the manuscript sources at the repository of the title footnote
and are submitted as supplementary material. The checks guard against
algebraic slips; no proof in the paper depends on them.

\section{Comparison with the Common-Reconstruction Functions of Lu \emph{et al.} and of Lu and Xu}
\label{app:lucompare}
...
sufficiency (Lemma~\ref{thm:pair}) is $L$. The binary conditional-content
...
$h_2(p)-h_2(D)$ (their Remark~3), the $q=0$ case of Theorem~\ref{thm:binary}; at
...
constrain. The joint $(R,L)$ region and the tilt parametrization of the
whole frontier (Proposition~\ref{rem:binfrontier}) have no counterpart in
\cite{lu2016lossy}: they bound both coordinates of one description at
once, rather than the single common-reconstruction rate, and are the
binary contribution of this paper.

\subsection*{Lu and Xu's Gaussian function}

Lu and Xu's vector Gaussian Heegard--Berger problem with common
reconstructions \cite{luxu2021} is the closest Gaussian counterpart. Their Theorem~4 gives the rate--distortion function
for two decoders with side information under error-covariance
constraints $\Cov(X-\hX)\preceq\mathbf D_m$, with $\mathbf D_m$ diagonal
and positive definite, under the hypothesis $\mathbf D_m\preceq\Sigma_X$
in the positive semidefinite order. Specialized to one decoder with side
information $S$ and constraint $\mathrm{diag}(D,D_V)$ on the pair
$(Y,V)$, with $s=1+\tau^2$, it reads
$\tfrac12\log_2\bigl[(1-\rho^2)(\tau^2+D_V)/(sDD_V)\bigr]$: their
Assumption~1 holds in the $(Y,V)$ coordinates, since
$\Sigma_{T\mid S}^{-1}-\Sigma_T^{-1}=\mathrm{diag}(0,\tau^{-2})$, and the
same specialization returns Steinberg's scalar formula when the context
is dropped. The present problem is the unpenalized-context limit $D_V\to\infty$ of
the underlying Lu--Xu constraint family,
at which the covariance constraint reduces to $\E(Y-\hY)^2\le D$; every
channel feasible for their constraint is feasible here, and dropping
$\hat V$ lowers the conditional information by data processing, so
their function is at least $L(D)$ wherever it is defined. What their
closed form does not cover is that limit: its hypothesis
$\mathbf D_m\preceq\Sigma_X$ forces $(1-D)(1-D_V)\ge\rho^2$, which leaves
no admissible $D_V$ unless $D<1-\rho^2$ and never leaves the context
unpenalized. On the admissible range the inequality is strict: the specialized
value decreases in $D_V$, and at the largest admissible $D_V$ it equals
$\tfrac12\log_2g_R$ with
$P(g_R)=\rho^2\tau^4(1-D)(1-\rho^2)/\bigl(s(1-D-\rho^2)^2\bigr)>0$, so
$g_R\ne g^\star$ while $g_R\ge g^\star$ by the inequality just proved.
The formal continuation of the formula to $D_V=\operatorname{Var}V$
equals Gray's clean-context function
$\tfrac12\log_2\bigl((1-\rho^2)/D\bigr)$, strictly below $L(D)$ whenever
$\rho\tau\ne0$, falling further beyond. Theorem~\ref{thm:function}(a)
is therefore the value of a limit that their closed form is not stated
for and does not continue to.

\section*{Acknowledgment}

In accordance with the IEEE policy on AI-generated text, the author
discloses the following use of Anthropic's Claude models (Claude Opus
4.8, Claude Opus 5, Claude Fable 5, and Claude Fable 5.1), run through
the Claude Code environment between August and September 2026. The
models drafted first versions of the prose and of the proofs in every
section and appendix, from the author's statements of the results and
proof outlines, and the author revised them. They carried out the
prior-art search summarized in the related-work part of
Section~\ref{sec:intro}, whose sources the author read. They wrote the
independent numerical check scripts and the reproduction notebook of
Appendix~\ref{app:numver} to the author's specification, and produced
the figures from the author's formulas. They were used for
copy-editing throughout and for five pre-submission review passes whose
findings the author adjudicated. Every theorem statement, proof, claim,
and number in the manuscript was checked by the author, who is
...
information for arbitrary ensembles,'' \emph{Inf. Control}, vol. 38,
no. 1, pp. 51--59, 1978. % verify vol/pages on final
...
\emph{IEEE Trans. Inf. Theory}, vol. 29, no. 6, pp. 918--923, 1983.
...
\bibitem{sagawa2009} T. Sagawa and M. Ueda, ``Minimal energy cost for
thermodynamic information processing: Measurement and information
erasure,'' \emph{Phys. Rev. Lett.}, vol. 102, art. 250602, 2009.
...
% The biography is omitted from the review manuscript, per the T-IT author
% information; delete the \iffalse ... \fi switch for the final version.
\iffalse
