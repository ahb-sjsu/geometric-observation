...
units of $\kb T\ln 2$. Three results are primary. First, a closed form:
...
the determinant bound, without a conditioner & A conditioner (third-party side information) present;
...
to the retained side information (the conditional content of the
title and abstract); the term is ours, chosen for the
...
\emph{Observation-theoretic reading.} Theorem~\ref{thm:pair} identifies
a lower-dimensional observationally sufficient representation of the
ambient source. The source $X$ may have any dimension $d$, but every
distinction outside $T=(Y,V)$ is operationally irrelevant to the
conditional-content optimization, and the function itself sees $X$ only
through $(\rho,\tau^2)$. This is one concrete instance of a
consumer-relative notion of effective observational dimension, taken up
in \cite{bond2026ot}. Nothing below depends on that reading.
...
the reset work per symbol, $\kb T\ln2\,[\Bdet]^{+}$ ($T$ here the
temperature of Remark~\ref{rem:thermo}), the positive part
...
$\Sigma_{\hat{\mathbf Y}}=\Cov(\hat{\mathbf Y})$, set
$A:=C^{\mathsf T}\Sigma_T^{-1}$ and
$\Sigma_N:=\Sigma_{\hat{\mathbf Y}}-A\Sigma_TA^{\mathsf T}\succeq0$ (assume
$\Sigma_T\succ0$). Degenerate instances reduce by dropping dependent
context components, and singular $\Sigma_N$ is dispatched by
Remark~\ref{rem:degenerate}: a singular residual either reveals
...
$E\sim\mathcal N(0,\Delta)$ independent with
$\widetilde Y=\hat{\mathbf Y}'+E$ (the reverse channel; the required
...
$E':=\widetilde Y-\hat{\mathbf Y}'$. In the chain
\[
\Bdet=I(\widetilde Y;\hat{\mathbf Y}')
=h(\widetilde Y)-h\bigl(E'\mid\hat{\mathbf Y}'\bigr)
\ \ge\ h(\widetilde Y)-h(E')
\ \ge\ h(\widetilde Y)-h(E'_A)-h(E'_B)
\ \ge\ \Bdet,
\]
equality in conditioning-reduces-entropy, in subadditivity, and in the
Gaussian maximum-entropy bound forces $E'\perp\hat{\mathbf Y}'$ with
$E'\sim\mathcal N(0,\Delta)$ (the same three-step equality analysis
...
$\tau^2>0$ the chain is
$L\ge I(Y;\hat{\mathbf Y}\mid S)=h(Y\mid S)-h(E\mid\hat{\mathbf Y},S)
\ge h(Y\mid S)-h(E)\ge h(Y\mid S)-h(E_A)-h(E_B)\ge\Bdet$ with
$E=Y-\hat{\mathbf Y}$, and attainment forces every inequality to
equality. Equality in conditioning-reduces-entropy makes $E$
independent of $(\hat{\mathbf Y},S)$; the finiteness this step needs is
automatic along the chain, since $h(E\mid\hat{\mathbf Y},S)=-\infty$
would make the conditional content infinite rather than equal to
$\Bdet$. Equality in subadditivity makes $E_A\perp E_B$. Equality in
the Gaussian maximum-entropy bound makes each $E_i$ Gaussian with
variance exactly $D_i$, the distortion constraints active. Together
these pin the error law, $E\sim\mathcal N(0,\Delta)$ independent of
...
channel attains $L=\Bdet$. Write $E:=Y-\hat{\mathbf Y}$. The chain
\eqref{eq:floor} reads, term by term,
\[
\Bdet=L\;\ge\;I(Y;\hat{\mathbf Y}\mid S)
=h(Y\mid S)-h\bigl(E\mid\hat{\mathbf Y},S\bigr)
\;\ge\;h(Y\mid S)-h(E)
\;\ge\;h(Y\mid S)-h(E_A)-h(E_B)
\;\ge\;\Bdet,
\]
using $h(Y\mid\hat{\mathbf Y},S)=h(E\mid\hat{\mathbf Y},S)$
(translation by the conditioning variable),
conditioning-reduces-entropy, subadditivity, and the Gaussian maximum-entropy bound
$h(E_i)\le\tfrac12\log_2\bigl(2\pi e\operatorname{Var}E_i\bigr)
\le\tfrac12\log_2(2\pi eD_i)$. Equality throughout forces:
(i) $h(E\mid\hat{\mathbf Y},S)=h(E)$, so
$I\bigl(E;(\hat{\mathbf Y},S)\bigr)=0$ and $E$ is independent of
$(\hat{\mathbf Y},S)$;
(ii) $h(E)=h(E_A)+h(E_B)$, so $E_A\perp E_B$;
(iii) each $E_i$ is Gaussian with variance exactly $D_i$.
Hence $E\sim\mathcal N(0,\Delta)$ independent of
...
level: $\Cov E=\Delta$, $\Cov(E,\hat{\mathbf Y})=0$, $\Cov(E,S)=0$;
also $\Cov(E,U)=0$ ($E$ is a function of $X$ and channel noise, $U$ is
independent of both), so $\Cov(E,V)=\Cov(E,S)-\Cov(E,U)=0$ and
$\Cov(\hat{\mathbf Y},V)=\Cov(Y,V)-\Cov(E,V)=\beta$. Moreover
$\Cov(Y,E)=\Cov(\hat{\mathbf Y}+E,E)=\Delta$, so
\[
\Cov(\hat{\mathbf Y})=\Cov(Y-E)
...
$E\sim\mathcal N(0,\Delta)$ independent, $Y=\hat{\mathbf Y}+E$, which
...
$E\sim\mathcal N(0,\Delta)$ jointly Gaussian with
$\Cov(E,Y)=\Delta$, $\Cov(E,V)=0$; such a $E$ exists iff
$\Delta-\Delta\Sigma_{Y\mid V}^{-1}\Delta\succeq0
\Leftrightarrow\Delta\preceq\Sigma_{Y\mid V}$ (Schur complement). The
channel $\hat{\mathbf Y}=Y-E$ yields
...
shadow of condition (a): for $\tau=0$ and $D>1-\rho^2$ the clipped bound
...
Every check passes: the symbolic identities are exact, the numerical
residuals sit at machine precision or at the optimizer's tolerance, and
the Lean and MATLAB checks close with no open goals.}
\label{tab:numver}
\centering
\footnotesize
