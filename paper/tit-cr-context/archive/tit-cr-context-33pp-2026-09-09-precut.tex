...
scalar Gaussian formula at its corners. The doubly symmetric binary case
is solved through a tilt equation, and coding theorems, including a
...
asymmetry creates: in closed form for Gaussian sources, and through a
tilt equation for the doubly symmetric binary source.
...
the conditional-variance lower bound is attained: the condition involves the encoder-accessible
conditional variance, not the third party's. Three extensions follow: the binary
...
Fig.~\ref{fig:frontiermulti} shows the resulting tradeoff at three
correlations.
...
here\\
\hline
Closed form (Theorem~\ref{thm:function}(a)) & Steinberg
...
channel\\
Rate--content region (Theorem~\ref{thm:region}(b)) & \cite{steinberg2009,
...
Proposition~\ref{rem:cleanboundary}) & none known to us & & Strict tradeoff iff
...
is dropped. The hypothesis $\mathbf D\preceq\Sigma_X$ forces
$(1-D)(1-D_V)\ge\rho^2$, so the context coordinate is never unpenalized
in their setting, and on the admissible range the specialized function
exceeds $L(D)$, the constraint on $V$ binding (checked numerically over
ten thousand instances, Appendix~\ref{app:numver}). Its formal extension
to $D_V=\operatorname{Var}V$ equals Gray's clean-context function
$\tfrac12\log_2\bigl((1-\rho^2)/D\bigr)$, a strict lower bound on $L(D)$
whenever $\rho\tau\ne0$, and it falls below that bound beyond; the
extension is therefore not valid, and Theorem~\ref{thm:function}(a) is
neither a specialization nor a limit of \cite{luxu2021}. The
difference is the constraint itself: a bound on the error of one linear
functional of the source is not a diagonal covariance constraint in any
coordinates.
...
rate--distortion theory for secrecy systems \cite{schieler2014}. Nearest
on the encoder side is Yamamoto's problem of a source with an additional
output to be kept secret \cite{yamamoto1983}: there too the encoder
observes the pair, but the priced quantity is the equivocation of the
additional output at the receiver or wiretapper, not that of the
description at a party holding a noisy copy of it. Closer in
...
differences separate the present problem from the secrecy line. Its results price the
...
rate--content region (Theorem~\ref{thm:region}(b)), the strict-misalignment
...
reading, its setting, and its scope.

A non-common-reconstruction baseline is remote source coding with
...
conditioner here rather than a reconstruction resource.

The relation to the information bottleneck
...
by the author, archived at doi:10.5281/zenodo.21776291. The operational region theorem for discrete sources
...
its frontier, the attainment condition of Remark~\ref{rem:floor}, vector context, the
...
isothermal protocols conditioned on $S^n$, so the infimum of the
...
vector, and the unit vector with $Y=y_0^{\mathsf T}Z$, $Z=WT$ the whitened pair of \eqref{eq:whitening}
...
maximum-entropy bound at variance $D$. On the active regime
$D\le\operatorname{Var}(Y\mid V)=1-\rho^2$ the bound is attained iff
$\rho\tau=0$: substituting $g_f$ into \eqref{eq:Pg} gives
$P(g_f)=-\rho^2\tau^2/s$, negative exactly when $\rho\tau\ne0$, in which
case $g^\star>g_f$ strictly. The variance that decides attainment is
thus the encoder-accessible $\operatorname{Var}(Y\mid V)$, through the
regime condition, and not the third party's $\operatorname{Var}(Y\mid S)$,
the same substitution as in Proposition~\ref{prop:marg}.
\end{remark}
...
of the system's solution with $\nu>0$, at every $\alpha\in[0,1]$ is
...
stationary point with $\nu>0$ is that minimizer); its proof is
dimension-free.
...
frontier of the binary rate--content region is the lower-left envelope
of the curve
\[
...
frontier is the lower-left envelope of the displayed curve along the
segment. On the segment, $R$ is strictly convex
...
(Remark~\ref{rem:thermo}), $L(D)$ is the infimum of the average erasure work per symbol, in
units of $\kb T\ln2$, paid in the quasi-static limit by a reset
mechanism that retains $S$. The region states when storing
